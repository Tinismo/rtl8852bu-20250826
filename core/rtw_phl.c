...
static void _hw_ic_info_cfg(struct dvobj_priv *dvobj, struct rtw_ic_info *ic_info)
{
	...
	if (dvobj->interface_type == RTW_HCI_USB) {
		...
		ic_info->usb_info.usb_support_interrupt= usb->usb_support_interrupt;
	}
	#endif
...
#ifdef CONFIG_NAN
static void rtw_core_set_nan_ops(struct dvobj_priv *d)
{
	struct rtw_phl_nan_ops ops = {0};

	ops.fill_ies = rtw_core_nan_fill_ies;

	ops.tx_mgnt = rtw_core_nan_issue_mgnt_frame;
	ops.get_attr_start = rtw_core_nan_get_attr_start;
	ops.get_frame_body = rtw_core_nan_get_frame_body;

	ops.report_to_osdep = rtw_core_nan_report_to_osdep;
	ops.alloc_peer_sta_info = rtw_core_nan_alloc_peer_sta_info;
	ops.free_peer_sta_info = rtw_core_nan_free_peer_sta_info;
#ifdef CONFIG_NAN_R2
	ops.set_pairwise_key = rtw_core_nan_set_pairwise_key;
	ops.pmk_to_ptk = rtw_core_nan_pmk_to_ptk;
	ops.get_key_mic = rtw_core_nan_get_key_mic;
	ops.get_auth_token = rtw_core_nan_get_auth_token;
	ops.get_pmkid = rtw_core_nan_get_pmkid;
#endif
	rtw_phl_nan_init_ops(GET_PHL_INFO(d), &ops);
}
#endif
...
int rtw_hw_connect_remove_sta(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	status = rtw_phl_cmd_update_media_status(phl, sta->phl_sta, NULL, false,
							PHL_CMD_DIRECTLY, 0);
	if (status != RTW_PHL_STATUS_SUCCESS) {
		RTW_ERR(FUNC_ADPT_FMT ": update media status fail(0x%x)!\n",
					FUNC_ADPT_ARG(a), status);
		return -1;
	}
...
int rtw_hw_connected(struct _ADAPTER *a)
{
	...
	void *phl;
	enum rtw_phl_status status;
	struct _ADAPTER_LINK *plink = GET_PRIMARY_LINK(a);
	struct _ADAPTER_LINK *alink = NULL;
	struct sta_info *sta = NULL, *psta = NULL;
	u8 *mac_addr = NULL;
	u8 lidx;
	...
	mac_addr = (u8*)plink->mlmeextpriv.mlmext_info.network.MacAddress;
	psta = rtw_get_stainfo(&a->stapriv, mac_addr);
	if (psta == NULL) {
		RTW_ERR(FUNC_ADPT_FMT ": drv sta_info(" MAC_FMT ") not exist!\n",
			FUNC_ADPT_ARG(a), MAC_ARG(mac_addr));
		return -1;
	}

	for (lidx = 0; lidx < a->adapter_link_num; lidx++) {
		alink = GET_LINK(a, lidx);
		mac_addr = (u8*)alink->mlmeextpriv.mlmext_info.network.MacAddress;
		if (IS_ACCEPTED_LINK(alink)) {
			if (alink == plink) {
				sta = psta;
			} else {
				sta = rtw_get_stainfo(&a->stapriv, mac_addr);
				if (!sta) {
					RTW_ERR(FUNC_ADPT_FMT ": drv sta_info(" MAC_FMT ") not exist!\n",
						FUNC_ADPT_ARG(a), MAC_ARG(mac_addr));
					return -1;
			...
			if (rtw_sta_hal_media_status_rpt_cmd(a, sta, true, RTW_CMDF_DIRECTLY) != _SUCCESS) {
				RTW_ERR(FUNC_ADPT_FMT ": update media status fail!\n", FUNC_ADPT_ARG(a));
				return -1;
			}
		...
		else {/* Remove the unaccepted link sta */
			sta = rtw_get_stainfo(&a->stapriv, mac_addr);
			if (sta) {
				/*rtw_hw_connect_remove_sta(a, sta);*/

				rtw_phl_chanctx_del(adapter_to_dvobj(a)->phl, a->phl_role,
						   alink->wrlink, NULL);
	...
	status = rtw_phl_connected(phl, a->connect_bidx, a->phl_role, psta->phl_sta);
...
static u8 _cfg_mdns_ofld_info(struct _ADAPTER *a)
{
	...
	info->mdns_en = (info->offload_state) ? true : false;
...
#define DBG_USB_PLATFORM_API 1 /* Can change to 0 to debug no platform api case */

#if DBG_USB_PLATFORM_API
extern int rtk_usb_phy_mode_switch(int mode);
extern int rtk_usb_phy_mode_save(int mode);
extern int rtk_read_usb_phy_mode_emmc_state(void);
#endif

int rtw_scan_switch_usb_mode(_adapter *padapter)
{
	...
	bool support_middle_perf_mode = _FALSE;
#ifdef CONFIG_BTC
	bool need_bt = _TRUE;
#else
	bool need_bt = _FALSE;
	...
	bool need_p2p = _TRUE;
#else
	bool need_p2p = _FALSE;
#endif
#if defined(CONFIG_80211AC_VHT) || defined(CONFIG_80211AX_HE)
	bool need_hight_tp = _TRUE;
#else
	bool need_hight_tp = _FALSE;
#endif
	u32 ability = 0;
	u32 mode_status = 0;
	u32 perf_mode = 0;
	...
	if (registry_par->switch_usb_mode != 0) {
		RTW_WARN("%s, registry_par->switch_usb_mode not 0 skip this!!\n",
			__func__);
		return _FALSE;
	}

	psts = rtw_phl_cmd_get_usb_support_ability(dvobj->phl,
						   &ability,
						   HW_BAND_0,
						   PHL_CMD_DIRECTLY,
						   0);

	if (psts == RTW_PHL_STATUS_SUCCESS && ability == 0x1 && try_count > 0) {
#if DBG_USB_PLATFORM_API
		if (rtk_read_usb_phy_mode_emmc_state() == 0)/* get usb host mode by platform api */
#endif
		{
			RTW_INFO("%s, wifi ability and host mode not match try again(%d)\n",
				__func__, try_count);
			try_count--;
		}
	} else {
		rtw_phl_cmd_get_usb_mode_status(dvobj->phl, &mode_status, HW_BAND_0,
						PHL_CMD_DIRECTLY, 0);
		if (mode_status == USB_MODE_U2_SWITCH_FAIL ||
		    mode_status == USB_MODE_U2_ENV_LIMIT) {
			RTW_ERR("%s, Switch USB mode fail, keep in U2 mode!!\n", __func__);
			return _FALSE;
		}
	}
	psts = rtw_phl_cmd_get_u3_perf_mode(dvobj->phl, &perf_mode, HW_BAND_0,
					   PHL_CMD_DIRECTLY, 0);

	RTW_INFO("%s, perf_mode=%d, psts(%d)\n", __func__, perf_mode, psts);

	if (psts == RTW_PHL_STATUS_SUCCESS && perf_mode == U3_PERF_MIDDLE)
		support_middle_perf_mode = _TRUE;

	if ((pusb_data->usb_speed < RTW_USB_SPEED_SUPER &&
		!need_bt && !need_p2p && need_hight_tp && !support_middle_perf_mode) ||
	    (pusb_data->usb_speed < RTW_USB_SPEED_SUPER &&
		need_hight_tp && support_middle_perf_mode) ||
	    (pusb_data->usb_speed >= RTW_USB_SPEED_SUPER  &&
		!support_middle_perf_mode)) {
		...
		pmlmepriv->scan_24g_exist = _FALSE;
		pmlmepriv->in_probe_chk_24g = _TRUE;
		rtw_drv_scan_by_self(padapter, RTW_AUTO_SCAN_REASON_CHECK_24G,
				     false);
		while (wait_timeout != 0 && check_fwstate(pmlmepriv, WIFI_UNDER_SURVEY)) {

			if (pmlmepriv->scan_24g_exist) {
		...
		pmlmepriv->in_probe_chk_24g = _FALSE;
		RTW_INFO("%s, pmlmepriv->scan_24g_exist=%d, wait_timeout=%d\n",
			 __func__, pmlmepriv->scan_24g_exist, wait_timeout);

		if (pusb_data->usb_speed < RTW_USB_SPEED_SUPER) {
			if (!pmlmepriv->scan_24g_exist && !need_bt && !need_p2p) {
				rtw_phl_cmd_force_usb_switch(dvobj->phl,
							     RTW_USB_SPEED_SUPER,
							     HW_BAND_0,
							     PHL_CMD_DIRECTLY,
							     0);
				return _TRUE;
			} else if (support_middle_perf_mode) {
#ifdef RTW_USB_CHG_HW_SET
#if DBG_USB_PLATFORM_API
				rtk_usb_phy_mode_save(1); /* save usb host mode by platform api */
#endif
				psts = rtw_phl_cmd_get_usb_support_ability(dvobj->phl,
									   &ability,
									   HW_BAND_0,
				...
				if (psts == RTW_PHL_STATUS_SUCCESS && ability == 0x2) {

					psts = rtw_phl_custom_usb_set_support_ability(dvobj->phl,
										      0x1,
										      HW_BAND_0,
										      PHL_CMD_DIRECTLY,
										      0);
					if (psts != RTW_PHL_STATUS_SUCCESS)
						RTW_WARN("%s, Set usb support ability fail(%d)\n", __func__, psts);

				}
#endif
#if DBG_USB_PLATFORM_API
				rtk_usb_phy_mode_switch(1); /* switch usb host mode by platform api */
#endif
				rtw_phl_cmd_force_usb_switch(dvobj->phl,
							     RTW_USB_SPEED_SUPER,
							     HW_BAND_0,
