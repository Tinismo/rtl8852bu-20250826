...
void WMMOnAssocRsp(_adapter *padapter, struct _ADAPTER_LINK *padapter_link)
{
	...
	if (WIFI_ROLE_LINK_IS_ON_5G(padapter_link) ||
	    WIFI_ROLE_LINK_IS_ON_6G(padapter_link) ||
...
#define BCNKEY_VERIFY_WHOLE_RATE_SET 0
	...
	if (!rtw_is_bchbw_grouped(cur->band, cur->ch, cur->bw, cur->offset,
				  recv->band, recv->ch, recv->bw,
				  recv->offset)) {
#ifdef CONFIG_CFG80211_SME_OFFLOAD
		if (cur->ch == 0) {
			RTW_INFO("sme offload bypass check bcn info first time\n");
			ret = _TRUE;
		}
#endif
		goto exit;
	...
	if (check_fwstate(pmlmepriv, WIFI_CSA_UPDATE_BEACON)) {
		...
		*/
		...
		if (psta) {
			...
			rtw_sta_hal_ra_mask_update_cmd(adapter, psta, 0);
		}
...
void process_csa_ie(_adapter *padapter, u8 *ies, uint ies_len)
{
	...
	if (csa_ch != 0) {
		...
		if (csa_band == BAND_MAX)
			csa_band = (csa_ch > 14) ? BAND_ON_5G : BAND_ON_24G;
...
unsigned int is_ap_in_tkip(struct _ADAPTER_LINK *padapter_link)
{
	...
	if (rtw_get_capability((WLAN_BSSID_EX *)cur_network) & WLAN_CAPABILITY_PRIVACY) {
		for (i = _FIXED_IE_LENGTH_; i < pmlmeinfo->network.IELength;) {
...
unsigned int should_forbid_n_rate(_adapter *padapter)
{
	...
	if (rtw_get_capability((WLAN_BSSID_EX *)cur_network) & WLAN_CAPABILITY_PRIVACY) {
		for (i = _FIXED_IE_LENGTH_; i < cur_network->IELength;) {
 ...
 *
 */
void rtw_set_mac_addr_hw(_adapter *padapter, const u8 *mac_addr)
{
