...
void expire_timeout_post_chk(_adapter *padapter, u32 ap_chk_sta_bmp)
{
	...
	if (padapter->mlmeextpriv.active_keep_alive_check) {
		...
		if (switch_channel_by_drv == _TRUE && bk_chdef.chan > 0)
			set_bch_bwmode(padapter, padapter_link, bk_chdef.band,
				       bk_chdef.chan, bk_chdef.offset, bk_chdef.bw,
...
static void rtw_ap_check_scan(_adapter *padapter, struct _ADAPTER_LINK *padapter_link)
{
	...
	while (1) {
		...
		if (rtw_chset_search_bch(adapter_to_chset(padapter)
...
void rtw_core_ap_swch_start(_adapter *padapter, struct createbss_parm *parm)
{
	...
	enum rfk_tri_type do_rfk = RFK_TYPE_FORCE_NOT_DO;
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	if (lmlmeextpriv->do_rfk_ap == _TRUE) {
		lmlmeextpriv->do_rfk_ap = _FALSE;
		do_rfk = RFK_TYPE_FORCE_DO;
		RTW_INFO("%s, do_rfk=%d\n", __func__, do_rfk);
	}
	...
	if (parm->ch_to_set != 0) {
		set_bch_bwmode(padapter, padapter_link, parm->req_band,
			parm->ch_to_set, parm->offset_to_set, parm->bw_to_set, do_rfk);
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	/* channel and band */
	rtw_ies_get_chdef(ie + _BEACON_IE_OFFSET_, pbss_network->IELength - _BEACON_IE_OFFSET_, &chandef, 1, 1, 1, 1);
	if (chandef.chan != 0) {
		pbss_network->Configuration.Band = chandef.band;
		pbss_network->Configuration.DSConfig = chandef.chan;
	} else if (pbss_network->Configuration.DSConfig != 0) {
		chandef.band = pbss_network->Configuration.Band;
		chandef.chan = pbss_network->Configuration.DSConfig;
	} else
		rtw_warn_on(1);
	...
	if (REGSTY_IS_11AX_ENABLE(pregistrypriv)
		...
		&& ((chandef.band == BAND_ON_5G && pmlmepriv->vhtpriv.vht_option == _TRUE)
		|| (chandef.band == BAND_ON_24G && pmlmepriv->htpriv.ht_option == _TRUE)
		...
		if (he_cap == _TRUE) {
			...
			rtw_he_ies_attach(padapter, padapter_link, pbss_network, chandef.band);
		}
	}
	...
	rtw_bss_get_chdef(pbss_network, &pmlmepriv->ori_chandef, 1, 1, 1, 1);
...
u8 ap_free_sta(_adapter *padapter, struct sta_info *psta, bool active, u8 subtype, u16 reason, bool enqueue)
{
	...
	report_del_sta_event_ap(padapter, psta->phl_sta->mac_addr, reason, enqueue, _FALSE);
...
void rtw_ap_restore_network(_adapter *padapter)
{
	...
	set_bch_bwmode(padapter, padapter_link, pmlmeext->chandef.band, pmlmeext->chandef.chan,
		       pmlmeext->chandef.offset, pmlmeext->chandef.bw, RFK_TYPE_FORCE_NOT_DO);
...
void stop_ap_mode(_adapter *padapter)
{
	...
	if (chanctx_num && chan_def.chan != 0)
		set_bch_bwmode(padapter, chan_def.band, chan_def.chan, chan_def.offset, chan_def.bw, RFK_TYPE_FORCE_NOT_DO);
#endif
...
#define UPDATE_HT_CAP 1
	...
	if (bss->Configuration.DSConfig != chandef->chan) {
		p = rtw_get_ie((bss->IEs + _FIXED_IE_LENGTH_), _DSSET_IE_,
			&ie_len, (bss->IELength - _FIXED_IE_LENGTH_));
	...
	if (vhtpriv->vht_option == _TRUE) {
		...
		vht_cap_ie = rtw_get_ie((bss->IEs + _FIXED_IE_LENGTH_), WLAN_EID_VHT_CAPABILITY, &vht_cap_ielen, (bss->IELength - _FIXED_IE_LENGTH_));
		vht_op_ie = rtw_get_ie((bss->IEs + _FIXED_IE_LENGTH_), WLAN_EID_VHT_OPERATION, &vht_op_ielen, (bss->IELength - _FIXED_IE_LENGTH_));
...
#ifdef CONFIG_80211N_HT
	{
		...
		ht_cap_ie = rtw_get_ie((bss->IEs + _FIXED_IE_LENGTH_), EID_HTCapability, &ht_cap_ielen, (bss->IELength - _FIXED_IE_LENGTH_));
		ht_op_ie = rtw_get_ie((bss->IEs + _FIXED_IE_LENGTH_), EID_HTInfo, &ht_op_ielen, (bss->IELength - _FIXED_IE_LENGTH_));
	...
	, s16 req_ch, s8 req_bw, s8 req_offset, struct rtw_chan_def *chdef)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		rtw_bss_get_bchbw(network, &cur_ie_band[i], &cur_ie_ch[i]
			, &cur_ie_bw[i], &cur_ie_offset[i], 1, 1, 1, 1);
	...
	if (MSTATE_STA_LG_NUM(&mstate_others) || MSTATE_STA_LD_NUM(&mstate_others)
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(iface) && u_band == BAND_ON_24G)
				dec_bw[i] = rtw_mesh_adjust_2g_bw(u_ch, dec_bw[i], dec_offset[i]);
			#endif
		...
		if (ifbmp_all_mesh && u_band == BAND_ON_24G) {
			u_bw = rtw_mesh_adjust_2g_bw(u_ch, u_bw, u_offset);
			if (u_bw == CHANNEL_WIDTH_20)
...
update_bss_chbw:
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (dec_bw[i] > u_bw)
				dec_bw[i] = u_bw;
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(iface) && dec_band[i] == BAND_ON_24G)
				dec_bw[i] = rtw_mesh_adjust_2g_bw(dec_ch[i], dec_bw[i], dec_offset[i]);
			#endif
			if (dec_bw[i] == CHANNEL_WIDTH_20)
				dec_offset[i] = CHAN_OFFSET_NO_EXT;
			else
...
{
	if (forbid) {
		if (!LINK_MLME_IS_REGU_FORBID(alink)) {
			SET_LINK_MLME_STATE(alink, WIFI_REGU_FORBID);
			if (LINK_MLME_IS_ASOC(alink))
				rtw_sta_flush(alink->adapter, enqueue);
			rtw_link_hal_core_stop_beacon(alink, forbid, enqueue ? 0 : RTW_CMDF_DIRECTLY);
			return true;
		}
	} else {
		if (LINK_MLME_IS_REGU_FORBID(alink)) {
			_CLR_LINK_MLME_STATE_(alink, WIFI_REGU_FORBID);
			rtw_link_hal_core_stop_beacon(alink, forbid, enqueue ? 0 : RTW_CMDF_DIRECTLY);
			return true;
		}
	}
...
{
	return rtw_ap_link_regu_forbid_apply(alink
		, rtw_rfctl_is_regu_forbid_bss(adapter_to_rfctl(alink->adapter), ALINK_GET_BAND(alink))
		, enqueue);
}
#endif
