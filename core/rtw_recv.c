...
#ifdef CONFIG_NAN
	    rtw_phl_nan_is_nan_frame(adapter->dvobj->phl, ptr) == _FALSE &&
#endif
	    1)
		return _SUCCESS;
				...
				&& (rtw_wnm_try_btm_roam_imnt(adapter) > 0)
					...
					if (reason == 6 || reason == 7) {
						...
						if (pmlmepriv->smeofld_ap ||
							...
							cfg80211_rx_unprot_mlme_mgmt(adapter->pnetdev,
...
#ifdef CONFIG_NAN
	struct	sta_priv *pstapriv = &adapter->stapriv;
	u8 is_nan_data = rtw_phl_nan_is_nan_frame(adapter->dvobj->phl, ptr);
#endif
...
#endif
#ifdef CONFIG_NAN_R2
	if (_TRUE == is_nan_data &&
	    _TRUE == rtw_core_nan_rx_data_validate_hdr(adapter, precv_frame, &psta)) {
		goto pre_validate_status_chk;
	} else
#endif
...
#ifdef CONFIG_NAN
	rtw_phl_nan_replace_with_nan_mac(precvframe->u.hdr.adapter->dvobj->phl,
					 ptr, pattrib->dst);
#endif
...
u8 rtw_init_lite_recv_resource(struct dvobj_priv *dvobj)
{
	...
	u32 intin_buf_nr = RTW_INTINBUF_NR(dvobj);
	struct data_urb *intin_urb;
	struct trx_urb_buf_q *intin_urb_q = &dvobj->intin_urb_q;
	u32 intin_urb_nr = RTW_INTINURB_NR(dvobj);
#endif 
	...
	union recv_frame *prframe, struct rtw_recv_pkt *rx_req)
{
	...
	if (!is_bmc) {
		...
		if (rx_req->rx_rlink)
			prframe->u.hdr.adapter_link = GET_LINK(primary_padapter, rx_req->rx_rlink->id);
#ifdef CONFIG_NAN
		else if (rtw_phl_nan_is_nan_frame(GET_PHL_INFO(dvobj),
						  prframe->u.hdr.rx_data)) {
			_adapter *nan_adapter = wiphy_to_nan_adapter(dvobj_to_wiphy(dvobj));

			prframe->u.hdr.adapter = nan_adapter;
			prframe->u.hdr.pkt->dev = wdev_to_ndev(nan_adapter->rtw_wdev);
			/* ToDo CONFIG_RTW_MLD: currently primary link only */
			prframe->u.hdr.adapter_link = GET_PRIMARY_LINK(nan_adapter);
		}
#endif
		else
			prframe->u.hdr.adapter_link = GET_PRIMARY_LINK(primary_padapter);
...
#ifdef CONFIG_NAN
		if (!rtw_phl_nan_is_nan_frame(GET_PHL_INFO(dvobj),
					      prframe->u.hdr.rx_data))
#endif
		{
