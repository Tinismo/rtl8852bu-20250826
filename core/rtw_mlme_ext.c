...
struct action_handler OnAction_tbl[] = {
	...
	{RTW_WLAN_CATEGORY_PROTECTED_DUAL_PUBLIC, "ACTION_PROTECTED_PUBLIC", on_action_public},
...
#endif
	{RTW_WLAN_CATEGORY_UNP_S1G, "ACTION_S1G", &on_action_s1g}
};
...
int rtw_rfctl_init(struct dvobj_priv *dvobj)
{
	...
	rfctl->p2p_park_ch = 36; /* default set to CH36*/
#endif
...
void rtw_rfctl_update_op_mode(struct rf_ctl_t *rfctl, u8 ifbmp_mod, u8 if_op, u8 ifbmp_excl)
{
	...
	enum band_type band, u_band = BAND_MAX;
	u8 ch, bw, offset;
	u8 u_ch = 0;
	u8 u_bw = CHANNEL_WIDTH_MAX;
	u8 u_offset = CHAN_OFFSET_NO_DEF;
	bool notify = 0;
	int i;
...
int	rtw_init_link_mlme_ext_priv(struct _ADAPTER_LINK *padapter_link)
{
	...
	if ((pmlmeext->chandef.band != BAND_ON_24G) || ((padapter->registrypriv.wireless_mode & WLAN_MD_11B) == 0))
...
static void _mgt_dispatcher(_adapter *padapter, struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	...
	if (ptable->func) {
		 ...
		 * address or is nan frame.
		 */
		if (!rtw_match_self_addr(padapter, GetAddr1Ptr(pframe)) &&
#ifdef CONFIG_NAN
		    !rtw_phl_nan_is_nan_frame(padapter->dvobj->phl, pframe) &&
#endif
...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (LINK_MLME_IS_REGU_FORBID(padapter_link))
		return _SUCCESS;
...
#else /* !CONFIG_CFG80211_REPORT_PROBE_REQ */
...
#endif
#endif /* CONFIG_CFG80211_REPORT_PROBE_REQ */
#endif /* CONFIG_IOCTL_CFG80211 */

#if !defined(CONFIG_CFG80211_REPORT_PROBE_REQ)
#ifdef CONFIG_P2P
...
_continue:
	...
	if ((target_ie && (target_ielen == 4)) && (_TRUE == _rtw_memcmp((void *)target_ie, "Ozmo", 4))) {
		...
		printk("%s: Find Ozmo RC -- "MAC_FMT"  \n\n",
		       __func__, MAC_ARG(sa_addr));
	...
	if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _TRUE &&
		...
		if (psta == NULL) {
			...
			report_add_sta_event_ap(padapter, psta->phl_sta->mac_addr);
...
exit:
	return _SUCCESS;
#endif /* !defined(CONFIG_CFG80211_REPORT_PROBE_REQ) */
}
...
static void rtw_parsing_ap_info(struct _ADAPTER_LINK *padapter_link, u8 *pframe, u32 len)
{
	...
	/* clear mlmeext country and ch_info content*/
	if (is_alpha(pmlmeext->country[0]) && is_alpha(pmlmeext->country[1])) {
		memset(&pmlmeext->country, 0, sizeof(pmlmeext->country));
		memset(&pmlmeext->ch_info, 0, sizeof(pmlmeext->ch_info));
	}
	/* parsing country element */
	if (elems.country_info)
		parsing_country_element_ie(pmlmeext, elems.country_info - 2 , elems.country_info_len);

	/* clear mlmeext op_class and channel_list content*/
	if (pmlmeext->op_class) {
		pmlmeext->op_class = 0;
		memset(&pmlmeext->channel_list, 0, sizeof(pmlmeext->channel_list));
	}
	/* parsing AP Channel Report */
	if (elems.ap_channel_rpt)
		parsing_ap_channel_rpt_ie(pmlmeext, elems.ap_channel_rpt - 2 , elems.ap_channel_rpt_len);
}
...
unsigned int OnBeacon(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_match_bssid_ext(padapter, GetAddr3Ptr(pframe)))
	{
		if ((pmlmeinfo->state & WIFI_FW_AUTH_NULL)
			...
			if (pmlmepriv->smeofld_sta)
				pmlmeinfo->state &= ~WIFI_FW_WAIT_BEACON;
			else
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
				start_clnt_auth(padapter); /* start auth */
...
end_auth_null:
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				#ifdef CONFIG_80211D
				alink_process_regu_ies(padapter_link
					, pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_
					, len - (WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_));
				#endif
		...
		#if 0 /*#ifdef CONFIG_ADHOC_SUPPORT*/
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (padapter->mlmepriv.smeofld_ap) {
		return OnAuth_ApSmeOffloaded(padapter,
					     padapter_link,
					     pstat,
					     pframe,
					     len,
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	if (pmlmepriv->smeofld_sta) {
		return rtw_sme_onauth_client(padapter, padapter_link, pframe,
					     pkt_len, algthm, seq, status);
	}
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (padapter->mlmepriv.smeofld_ap) {
		...
		RTW_INFO("Inform rx assoc req on ch=%u, pkt_len=%u\n",
			 lmlmeextpriv->chandef.chan, pkt_len);
		rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
				     rtw_ch2freq(lmlmeextpriv->chandef.chan),
				     0, pframe, pkt_len, GFP_ATOMIC);
	}
#endif /* CONFIG_IOCTL_CFG80211 */
	...
	if (pstat && (pstat->state & WIFI_FW_ASSOC_SUCCESS) && (_STATS_SUCCESSFUL_ == status)) {
...
#endif
		{
			...
			{
				/* .2 issue assoc rsp
				 * before notify station join event. */
				if (frame_type == WIFI_ASSOCREQ) {
...
#endif /* CONFIG_IEEE80211W */
		{
			...
			report_add_sta_event_ap(padapter, pstat->phl_sta->mac_addr);
		}
#ifdef CONFIG_IEEE80211W
		if (pstat->bpairwise_key_installed == _TRUE && (pstat->flags & WLAN_STA_MFP)) {
			RTW_INFO(MAC_FMT"\n", MAC_ARG(pstat->phl_sta->mac_addr));
#ifdef CONFIG_CFG80211_SME_OFFLOAD
			if (!padapter->mlmepriv.smeofld_ap)
				/* As SME is offloaded to hostapd,
				 * it will do SA Query if
				 * necessary. See check_assoc_ies(). */
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
			{
				issue_action_SA_Query(padapter,
						      pstat->padapter_link,
						      pstat->phl_sta->mac_addr,
						      0,
						      0,
						      IEEE80211W_RIGHT_KEY);
			}
		}
#endif /* CONFIG_IEEE80211W */
...
#if defined(CONFIG_LAYER2_ROAMING) && (defined(CONFIG_RTW_80211K) || defined(CONFIG_RTW_FSM_RRM))
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	SET_ACCEPTED_LINK(padapter_link);
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	if (pmlmepriv->smeofld_sta)
		rtw_sme_onassocrsp(padapter, padapter_link, pframe, pkt_len);
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
	if (res > 0)
...
unsigned int OnDeAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (pmlmepriv->smeofld_ap ||
		...
		struct _ADAPTER_LINK *padapter_link = precv_frame->u.hdr.adapter_link;
		struct link_mlme_ext_priv *lpriv = &padapter_link->mlmeextpriv;

		RTW_INFO("driver_interface=%d\n", padapter->iface_id);
		RTW_INFO("Inform rx deauth on ch=%u, len=%u\n",
			 lpriv->chandef.chan, precv_frame->u.hdr.len);
		RTW_DBG_DUMP("OnDeAuth(): deauth:\n"
			     , pframe, precv_frame->u.hdr.len);
		if (pmlmepriv->smeofld_ap) {
			rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
					     rtw_ch2freq(lpriv->chandef.chan),
					     0, pframe, precv_frame->u.hdr.len,
					     GFP_ATOMIC);
		} else {
			cfg80211_rx_mlme_mgmt(padapter->pnetdev, pframe,
	...
	if (MLME_IS_AP(padapter)) {
		...
		if (psta) {
			...
			if (rtw_is_list_empty(&psta->asoc_list) == _FALSE) {
				...
				updated = ap_free_sta(padapter, psta, active, 0, reason, _TRUE);
...
unsigned int OnDisassoc(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (pmlmepriv->smeofld_ap ||
		...
		RTW_INFO("driver_interface=%d\n", padapter->iface_id);
		RTW_INFO("Inform rx disassoc on ch=%u, len=%u\n",
			 lpriv->chandef.chan, precv_frame->u.hdr.len);
		RTW_DBG_DUMP("OnDeAuth(): disassoc:\n",
			     pframe, precv_frame->u.hdr.len);
		if (pmlmepriv->smeofld_ap) {
			rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
					     rtw_ch2freq(lpriv->chandef.chan),
...
unsigned int on_action_wnm(_adapter *adapter, union recv_frame *rframe)
{
	...
	case RTW_WLAN_ACTION_WNM_BTM_REQ:
		if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
			...
			rtw_wnm_process_btm_req(adapter, sta, frame_body, frame_body_len);
		}
		ret = _SUCCESS;
		break;
#endif		
...
#ifdef CONFIG_NAN
	else if (_rtw_memcmp(frame_body + 2, NAN_OUI_SDF, 4) == _TRUE ||
#ifdef CONFIG_NAN_R2
		 _rtw_memcmp(frame_body + 2, NAN_OUI_NAF, 4) == _TRUE ||
#endif
		 0) {
		ret = rtw_phl_nan_on_action_public_nan(
				GET_PHL_INFO(adapter_to_dvobj(adapter)),
				pframe, frame_body, precv_frame->u.hdr.len,
				GetPrivacy(pframe));
	}
#endif /* CONFIG_NAN */
...
unsigned int on_action_public(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	/* check RA matches, broadcast or NAN frame */
	...
	if (category != RTW_WLAN_CATEGORY_PUBLIC &&
	    category != RTW_WLAN_CATEGORY_PROTECTED_DUAL_PUBLIC)
		goto exit;
...
unsigned int OnAction_ft(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_FT_RSP:
		...
		rtw_btm_notify_action_resp(psta->btm);
#else
...
unsigned int OnAction_sa_query(_adapter *padapter, union recv_frame *precv_frame)
{
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
		/*
		 * For SA Query Req, respond with Resp if in STA mode,
		 * otherwise report to cfg80211
		 */
		if (pmlmepriv->smeofld_ap &&
		    check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
			RTW_INFO("%s(): SME offload AP, report SA Query req\n",
				 __FUNCTION__);
			report = 1;
			break;
		}
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
		/*
		 * For SA Query Resp, always report to cfg80211
		 */
		if (pmlmepriv->smeofld_ap ||
		    pmlmepriv->smeofld_sta) {
			RTW_INFO("%s(): reporting SA Query resp to cfg80211\n",
				 __FUNCTION__);
			report = 1;
			break;
		}
	...
	if (report) {
		rtw_cfg80211_rx_mgmt(padapter->rtw_wdev,
				     rtw_ch2freq(lmlmeextpriv->chandef.chan),
				     0, precv_frame->u.hdr.rx_data,
				     precv_frame->u.hdr.len, GFP_ATOMIC);
	}
#endif
...
#elif defined(CONFIG_RTW_FSM_RRM)
	return rrm_on_action(padapter, precv_frame);
#else
	return _SUCCESS;
...
#ifdef CONFIG_RTW_MBO
		rtw_mbo_build_beacon_ies(padapter, pattrib->adapter_link, &pframe, &pattrib->pktlen);
#endif
#ifdef CONFIG_P2P
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
			u32 len = 0;
...
#ifdef CONFIG_RTW_MBO
	rtw_mbo_build_probe_rsp_ies(padapter, pattrib->adapter_link, &pframe, &pattrib->pktlen);
#endif
#ifdef CONFIG_P2P
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)
	...
	*(fctrl) = 0;
	...
	if (pregistrypriv->ht_enable == _TRUE
		...
		rtw_restructure_ht_ie(padapter, padapter_link, NULL, pframe, 0,
				      &len, rtw_get_oper_ch(padapter, padapter_link));
		pattrib->pktlen += len;
		pframe += len;
	}
#endif
...
#ifdef CONFIG_80211AC_VHT
	if ((pregistrypriv->ht_enable == _TRUE)
	     && (rtw_get_oper_band(padapter, padapter_link) == BAND_ON_5G)
		...
		len = rtw_build_vht_cap_ie(padapter, padapter_link, pframe);
		pattrib->pktlen += len;
		pframe += len;
	}
#endif
...
#ifdef CONFIG_80211AX_HE
	if (((pregistrypriv->ht_enable == _TRUE
		&& (rtw_get_oper_band(padapter, padapter_link) == BAND_ON_24G))
#ifdef CONFIG_80211AC_VHT
		|| (REGSTY_IS_11AC_ENABLE(pregistrypriv)
			&& (rtw_get_oper_band(padapter, padapter_link) == BAND_ON_5G))
#endif
		...
		len = rtw_build_he_cap_ie(padapter, padapter_link, pframe, rtw_get_oper_band(padapter, padapter_link));
		pattrib->pktlen += len;
		pframe += len;
	}
#endif
...
#ifdef CONFIG_RTW_MBO
	rtw_mbo_build_probe_req_ies(padapter, pattrib->adapter_link, &pframe, &pattrib->pktlen);
#endif
	pattrib->last_txcmdsz = pattrib->pktlen;
...
void issue_asocrsp(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	/* Adding RM capability IE */
	if (padapter->fsmpriv.rmpriv.enable == _TRUE) {
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _EID_RRM_EN_CAP_IE_, &ie_len, (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpy(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
...
#endif
#ifdef CONFIG_RTW_MBO
	rtw_mbo_build_assoc_rsp_ies(padapter, pattrib->adapter_link, &pframe, &pattrib->pktlen);
#endif
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
size_t rtw_build_assocreq_ies(_adapter *padapter,
			      struct _ADAPTER_LINK *padapter_link,
			      u8 *pframe, struct pkt_attrib *pattrib,
			      u8 is_reassoc, const u8 *ie, size_t ie_len)
#else
...
#endif
{
	...
	int ret = _SUCCESS;
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	int wpas_ie_len = ie_len;
	u8 *wpas_ie = NULL;
	int rrm_ie_len;
	u32 dyn_ie_len = 0, ie_index = 0;

	if (ie_len != 0) {
		wpas_ie = rtw_zmalloc(ie_len);
		_rtw_memcpy(wpas_ie, ie,ie_len);
	}
	...
	if (lmlmeext->chandef.band == BAND_ON_24G && lmlmeext->chandef.chan == 14)
		sta_bssrate_len = 4; /* for JAPAN, channel 14 can only uses B Mode(CCK) */
...
#ifdef CONFIG_RTW_MBO
	rtw_mbo_build_assoc_req_ies(padapter, &pframe, &pattrib->pktlen);
#endif
#ifdef CONFIG_RTW_80211R
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
		/*80211K depends on driver*/
		if (rtw_get_ie(wpas_ie, EID_RMEnabledCapability, &rrm_ie_len,
			       wpas_ie_len) != NULL) {
			rtw_ies_remove_ie(wpas_ie, &wpas_ie_len, 0,
					  EID_RMEnabledCapability, NULL, 0);
		}
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
	}
#endif /* CONFIG_RTW_80211K */
	...
	if (lmlmeinfo->network.PhyInfo.rm_en_cap[0] /* RM Enabled Capabilities */
		...
		| lmlmeinfo->network.PhyInfo.rm_en_cap[4]) {
		pframe = rtw_set_ie(pframe, _EID_RRM_EN_CAP_IE_, 5,
				(u8 *)padapter->fsmpriv.rmpriv.rm_en_cap_def, &(pattrib->pktlen));
#ifdef CONFIG_CFG80211_SME_OFFLOAD
		/*80211K depends on driver*/
		if (rtw_get_ie(wpas_ie, EID_RMEnabledCapability, &rrm_ie_len,
			       wpas_ie_len) != NULL) {
	...
	for (i = _FIXED_IE_LENGTH_; i < lmlmeinfo->network.IELength;) {
	...
	if (adapter_wdev_data(padapter)->p2p_enabled) {
...
#ifdef CONFIG_CONCURRENT_MODE
	else if (padapter->registrypriv.wifi_spec && (pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (!iface || iface == padapter || !rtw_is_adapter_up(iface))
				continue;

			if (!adapter_wdev_data(iface)->p2p_enabled || !iface->mlmepriv.p2p_beacon_ie
			    || iface->mlmepriv.p2p_beacon_ie_len == 0)
				continue;

			ptemp = pframe;
			/* copy P2P IE */
			_rtw_memcpy(pframe, iface->mlmepriv.p2p_beacon_ie, iface->mlmepriv.p2p_beacon_ie_len);
			pframe += iface->mlmepriv.p2p_beacon_ie_len;
			pattrib->pktlen += iface->mlmepriv.p2p_beacon_ie_len;

			/* get 2 bytes P2P CAPABILITY : P2P DEVICE CAPABILITY(1) + P2P GROUP CAPABILITY(1) */
			pattr = rtw_get_p2p_attr_content(ptemp, iface->mlmepriv.p2p_beacon_ie_len,
			                                 P2P_ATTR_CAPABILITY, NULL, NULL);

			/* clear P2P concurrent operation bit of P2P device capability
			 * for avoid association fail with Cisco AP
			 */
			if (pattr) {
				cap_attr = pattr;
				*cap_attr &= ~cpu_to_le16(P2P_DEVCAP_CONCURRENT_OPERATION);
			}
			break;
		}
	}
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	RTW_INFO_DUMP("wpa ie contents", wpas_ie, wpas_ie_len);
	if (wpas_ie_len != 0) {
		/* Append/replace additional IE */
		dyn_ie_len = pattrib->pktlen - ie_index;
		pframe = pframe - dyn_ie_len;
		rtw_append_ies_from_supplicant(&pframe , &dyn_ie_len, wpas_ie, 
					       wpas_ie_len);
		pattrib->pktlen = dyn_ie_len + ie_index;
	}
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
_exit:
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	if (ie_len != 0)
		rtw_mfree(wpas_ie, ie_len);

	return (size_t)pattrib->pktlen;
#else
	return ret;
#endif /* CONFIG_CFG80211_SME_OFFLOAD */
}
...
#ifdef CONFIG_CFG80211_SME_OFFLOAD
	frame_len = rtw_build_assocreq_ies(padapter, padapter_link, pframe,
					   pattrib, is_reassoc, NULL, 0);
...
#else /* !CONFIG_CFG80211_SME_OFFLOAD */
...
bool rtw_core_issu_null_data(void *priv, u8 ridx, u8 link_idx, bool ps, u8 module_id)
{
...
static int _issue_disasoc_deauth(_adapter *padapter, unsigned char *da
	, u8 disasoc, unsigned short reason, u8 wait_ack, u8 key_type)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
...
static int _issue_disasoc(_adapter *padapter, unsigned char *da, unsigned short reason, u8 wait_ack, u8 key_type)
{
	return _issue_disasoc_deauth(padapter, da, 1, reason, wait_ack, key_type);
}

int issue_disasoc(_adapter *padapter, unsigned char *da, unsigned short reason)
{
	RTW_INFO("%s reason(%u) to "MAC_FMT"\n", __func__, reason, MAC_ARG(da));
	return _issue_disasoc(padapter, da, reason, _FALSE, IEEE80211W_RIGHT_KEY);
}
				   ...
				   int wait_ms)
{
	...
	if (adapter_regu_block_tx(adapter))
		goto exit;
...
int issue_action_SM_PS_wait_ack(_adapter *padapter, unsigned char *raddr, u8 NewMimoPsMode, int try_cnt, int wait_ms)
{
	...
	if (adapter_regu_block_tx(padapter))
		goto exit;
...
unsigned int send_beacon(_adapter *padapter)
{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_OP_CH_SWITCHING | WIFI_REGU_FORBID))
		return _SUCCESS;
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if (!bssid->PhyInfo.isValid) {
		bssid->PhyInfo.SignalStrength = 30;
		bssid->PhyInfo.rssi = rtw_phl_rssi_to_dbm(bssid->PhyInfo.SignalStrength);
	}
...
void start_create_ibss(_adapter *padapter)
{
	...
	pmlmeext->chandef.band = (u8)pnetwork->Configuration.Band;
...
void start_clnt_join(_adapter *padapter)
{
	...
	struct mlme_ext_info *pinfosme = &(pmlmeextpriv->mlmext_info);
#endif
	...
	for (lidx = 0; lidx < padapter->adapter_link_num; lidx++) {
		...
		if (!IS_TO_JOIN_LINK(alink))
			continue;
		update_wireless_mode(padapter, alink);
		update_capinfo(padapter, alink, rtw_get_capability(&alink->mlmeextpriv.mlmext_info.network));
	}
	...
	if (caps & cap_ESS) {
...
#endif
		{
			...
			if (pmlmepriv->smeofld_sta) {
				pinfosme->state =
					pinfosme->state | WIFI_FW_STATION_STATE;
			} else
#endif
			{
				padapter->mlmeextpriv.mlmext_info.state =
					WIFI_FW_AUTH_NULL | WIFI_FW_STATION_STATE;
			}
		}
...
unsigned int receive_disconnect(_adapter *padapter, u8 *mac_addr, u16 reason, u8 locally_generated)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);

	if (!(rtw_match_bssid_ext(padapter, mac_addr)))
		return _SUCCESS;
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
			if (report_del_sta_event(padapter, mac_addr, reason, _TRUE, locally_generated) != _FAIL)
				pmlmeinfo->state = WIFI_FW_NULL_STATE;
...
static void rtw_check_24g_ap_exist(_adapter *adapter, WLAN_BSSID_EX *bss)
{
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;

	if (pmlmepriv->scan_24g_exist == _TRUE)
		return;
	if (bss->Configuration.Band == BAND_ON_24G) {
		pmlmepriv->scan_24g_exist = _TRUE;
		#if 0
		RTW_INFO("%s "MAC_FMT" %4s %3d %4d %s\n"
			, __func__
			, MAC_ARG(bss->MacAddress)
			, band_str(BSS_EX_OP_BAND(bss))
			, BSS_EX_OP_CH(bss)
			, bss->PhyInfo.rssi
			, bss->InfrastructureMode == Ndis802_11_mesh ? bss->mesh_id.Ssid : bss->Ssid.Ssid);
		#endif
	}
}
#endif
...
#ifdef CONFIG_SCAN_SWITCH_USB_MODE
	rtw_check_24g_ap_exist(padapter, bssid);
#endif
...
void report_surveydone_event(_adapter *padapter, u8 reason, bool complete, bool acs, u8 flags)
{
	...
	if (flags & RTW_CMDF_DIRECTLY) {
		...
		survey_done.reason = reason;
		survey_done.complete = complete;
		survey_done.acs = acs;
		RTW_INFO("survey done event(%x) reason:0x%x%s for "ADPT_FMT"-HW_BAND_%d\n",
			survey_done.bss_cnt,
			reason,
			complete ? " complete" : "",
			ADPT_ARG(padapter),
		...
		psurveydone_evt->reason = reason;
		psurveydone_evt->complete = complete;
...
u32 report_join_res(_adapter *padapter, int aid_res, u16 status)
{
	...
	if (pmlmeinfo->network.is_mld) {
		...
		for (lidx = 0; lidx < padapter->adapter_link_num; lidx++) {
			...
			if (!IS_TO_LINK_LINK(adapter_link) || adapter_link == padapter_link)
				continue;
			...
			if (!rec_link_network) {
				...
				CLEAR_TO_JOIN_LINK(adapter_link);
				continue;
			}
		...
		rtw_unlink_network_by_linkid(pmld_network, pjoinbss_evt->network.network.link_id);
		rtw_free_cloned_mld_network(pmld_network);
	} else
#endif
	{
		rtw_joinbss_event_prehandle(padapter, (u8 *)&pjoinbss_evt->network, status);
...
u32 report_del_sta_event_ap(_adapter *a, u8 *mac_addr, u16 reason, bool enqueue, u8 locally_generated)
{
	struct sta_info *psta;
	u8 res = _SUCCESS;

	rtw_warn_on(!MLME_IS_AP(a));

	if (enqueue) {
		res = rtw_add_del_sta_cmd_check(a, mac_addr, _FALSE);
	} else {
		psta = rtw_get_stainfo_to_free(&a->stapriv, mac_addr);
		if (!psta || !psta->phl_sta || !psta->phl_sta->mld) {
			res = _FAIL;
		} else {
			rtw_hw_disconnect(a, psta);
			res = rtw_free_mld_stainfo(a, psta->phl_sta->mld);
		}
	}
	RTW_INFO(FUNC_ADPT_FMT" "MAC_FMT" enqueue:%d, res:%u\n"
		, FUNC_ADPT_ARG(a), MAC_ARG(mac_addr),  enqueue, res);
	
	return res;
}

u32 report_del_sta_event(_adapter *padapter, u8 *mac_addr, u16 reason, bool enqueue, u8 locally_generated)
{
	struct cmd_obj *pcmd_obj;
	u8 *pevtcmd;
	u32 cmdsz;
	...
	rtw_warn_on(!MLME_IS_STA(padapter));
	...
	_rtw_memcpy((unsigned char *)(&(pdel_sta_evt->macaddr)), mac_addr, ETH_ALEN);
	_rtw_memcpy((unsigned char *)(pdel_sta_evt->rsvd), (unsigned char *)(&reason), 2);
	psta = rtw_get_stainfo_to_free(&padapter->stapriv, mac_addr);
	if (psta) {
...
exit:
	...
	RTW_INFO(FUNC_ADPT_FMT" "MAC_FMT" mac_id=%d, enqueue:%d, res:%u\n"
		, FUNC_ADPT_ARG(padapter), MAC_ARG(mac_addr), mac_id, enqueue, res);

	return res;
}
void report_add_sta_event_ap(_adapter *padapter, u8 *mac_addr)
{
	rtw_warn_on(!MLME_IS_AP(padapter));

	rtw_add_del_sta_cmd_check(padapter, mac_addr, _TRUE);
}

#if 0 /*#ifdef CONFIG_ADHOC_SUPPORT*/
void report_add_sta_event(_adapter *padapter, u8 *mac_addr)
{
	struct cmd_obj *pcmd_obj;
	u8 *pevtcmd;
	u32 cmdsz;
	...
	_rtw_memcpy((unsigned char *)(&(padd_sta_evt->macaddr)), mac_addr, ETH_ALEN);
...
void report_sta_timeout_event(_adapter *padapter, u8 *MacAddr, unsigned short reason)
{
	...
	RTW_INFO("%s: delete STA, mac_id=%d\n", __func__, mac_id);
...
u8 stop_ap_hdl(_adapter *adapter)
{
	...
	rtw_ap_link_regu_forbid_apply(adapter_link, false, false);
...
void update_join_link_info(struct _ADAPTER *a, struct _ADAPTER_LINK *alink, struct _WLAN_BSSID_EX *pbuf)
{
	...
	rtw_bss_get_chdef(pnetwork, &alink->mlmeextpriv.chandef, pmlmeinfo->HT_caps_enable,
			 pmlmeinfo->VHT_enable, pmlmeinfo->HE_enable, false);

	/*adjust chan/bw/offset with registary and hw cap*/
	rtw_adjust_chdef_bw(a, &alink->mlmeextpriv.chandef);
...
#define DUMP_ADAPTERS_STATUS 0
	...
	for (lidx = 0; lidx < adapter->adapter_link_num; lidx++) {
		...
		if (join_res >= 0 && IS_ACCEPTED_LINK(adapter_link)) { /*client join success - restart all ap*/
			for (i = 0; i < dvobj->iface_nums; i++) {
				iface = dvobj->padapters[i];
				mlme = &iface->mlmepriv;
				...
				for (iface_lidx = 0; iface_lidx < iface->adapter_link_num; iface_lidx++) {
					...
					if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
						...
						rtw_adjust_chdef_bw(iface, &lmlmeext->chandef);
						#ifdef DBG_DUMP_CHAN_SET
						...
						#endif
						#ifdef CONFIG_RTW_MESH
						if (MLME_IS_MESH(iface) && lmlmeext->chandef.band == BAND_ON_24G)
							lmlmeext->chandef.bw = rtw_mesh_adjust_2g_bw(lmlmeext->chandef.chan
								, lmlmeext->chandef.bw, lmlmeext->chandef.offset);
						#endif
						...
						rtw_ap_link_regu_forbid_update(iface_link, false);
...
static u8 join_cmd_pre_hdl(_adapter *padapter, WLAN_BSSID_EX *network)
{
	...
	if (psecnetwork == NULL) {
		RTW_ERR(FUNC_ADPT_FMT ": join_cmd_pre_hdl failed to alloc psecnetwork\n",
			FUNC_ADPT_ARG(padapter));
		goto exit;
	}
	...
	if (pmld_network) {
		for (lidx = 0; lidx < pmld_network->network_num; lidx++) {
			plink_network = GET_LINK_NETWORK(pmld_network, lidx);
			if (plink_network->network.link_id == network->link_id)
				continue;

			adapter_link = rtw_get_adapter_link_by_linkid(padapter, plink_network->network.link_id);
			if (!adapter_link || (adapter_link == padapter_link))
				continue;

			rtw_update_join_priv(padapter, adapter_link, &plink_network->network, psecnetwork);
		}
	}
#endif
	rtw_mfree((u8 *)psecnetwork, sizeof(WLAN_BSSID_EX));
...
u8 rtw_join_cmd_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (join_cmd_pre_hdl(padapter, &pparm->network) != _SUCCESS) {
		ret = H2C_CMD_FAIL;
		goto exit;
	}

	if (rtw_connect_cmd(padapter, &pparm->network) != RTW_PHL_STATUS_SUCCESS)
		ret = H2C_CMD_FAIL;
		...
		enum phl_cmd_type cmd_type,  u32 cmd_timeout)
{
	...
	RTW_PRINT("%s: set %s key for " MAC_FMT ", kid:%d type:%u algo:%s\n",
	  __func__, is_wep_enc(key->algorithm)?"WEP":"group", MAC_ARG(mac),
					...
					enum phl_cmd_type cmd_type,  u32 cmd_timeout)
{
	...
	if (key->algorithm == _NO_PRIVACY_) {
		RTW_INFO("%s: del all key for " MAC_FMT " for _NO_PRIVACY_\n",
				 __func__, MAC_ARG(key->addr));
		err = rtw_hw_del_all_key(a, sta, cmd_type, cmd_timeout);
	} else {
		RTW_INFO("%s: set %s key for " MAC_FMT ", kid:%d algo:%s\n",
				 __func__, key->gk?"group":"pairwise",
				 MAC_ARG(key->addr), key->keyid,
...
#define GEN_WLAN_EVT_HDL(id, data, cb) [id] = { .name = #id, .parmsize = sizeof(data), .event_callback = cb},

static const struct rtw_event wlanevents[] = {
	GEN_WLAN_EVT_HDL(EVT_SURVEY, struct survey_event, rtw_survey_event_callback)
	GEN_WLAN_EVT_HDL(EVT_SURVEY_DONE, struct surveydone_event, rtw_surveydone_event_callback)
	GEN_WLAN_EVT_HDL(EVT_JOINBSS, struct joinbss_event, rtw_joinbss_event_callback)
	/*GEN_WLAN_EVT_HDL(EVT_ADD_STA, struct stassoc_event, rtw_stassoc_event_callback)*/
	GEN_WLAN_EVT_HDL(EVT_DEL_STA, struct stadel_event, rtw_stadel_event_callback)
	GEN_WLAN_EVT_HDL(EVT_WMM_UPDATE, struct wmm_event, rtw_wmm_event_callback)
#ifdef CONFIG_IEEE80211W
	GEN_WLAN_EVT_HDL(EVT_TIMEOUT_STA, struct stadel_event, rtw_sta_timeout_event_callback)
#endif
#ifdef CONFIG_RTW_80211R
	GEN_WLAN_EVT_HDL(EVT_FT_REASSOC, struct stassoc_event, rtw_ft_reassoc_event_callback)
#endif
};
...
u8 mlme_evt_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (wlanevents[evt_hdr->id].event_callback) {
...
u8 tdls_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (!_rtw_memcmp(TDLSoption->addr, zaddr, ETH_ALEN)) {
		...
		if ((ptdls_sta == NULL) || (ptdls_sta->phl_sta == NULL))
			return H2C_REJECTED;
	}
	...
	case TDLS_ESTABLISHED:
		...
		if (!ptdls_sta->phl_sta->active) {
			RTW_INFO("%s - rtw_alloc_stainfo_hw for option %d\n", __func__, option);
		...
		rtw_sta_hal_media_status_rpt_cmd(padapter, ptdls_sta, true, RTW_CMDF_DIRECTLY);
...
int rtw_sae_preprocess(_adapter *adapter, const u8 *buf, u32 len, u8 tx)
{
	 ...
	 * 0x4d(77) : FINITE_CYCLIC_GROUP_NOT_SUPPORTED
	 * WPAS needs to resend AUTH req. with the token,
	 * driver must pass 0x4c to WPAS to handle it. */
	if (status == 0 || status == 0x7e || status == 0x4c || status == 0x4d)
		return _SUCCESS;
exit:
	return ret;
#else
...
const char *rtw_evt_name(struct rtw_evt_header *pev)
{
