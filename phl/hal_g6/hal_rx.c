                        ...
                        enum phl_phy_idx phy_idx, u8 *mode)
{
	...
	u8 prev_mode = 0, tgt_mode = 0;

	prev_mode = rtw_hal_get_rxfltr_opt_mode(hinfo, phy_idx);
	tgt_mode = *mode;
	hal_status = rtw_hal_set_rxfltr_opt_by_mode(hinfo,
			phy_idx, tgt_mode);

	if (tgt_mode != RX_FLTR_OPT_MODE_SCAN &&
	    tgt_mode != RX_FLTR_OPT_MODE_SCAN_MULTICAST &&
	    tgt_mode != RX_FLTR_OPT_MODE_SNIFFER)
	    PHL_ERR("%s :: unknown scan opt_mode (%d)\n", __func__, tgt_mode);
...
u32 rtw_hal_handle_usb_interrupt_buffer(void *h, u8 *buf, u32 buf_len)
{
	...
	interrupted = trx_ops->handle_usb_interrupt_buffer(hal_info, buf, buf_len);
	return interrupted;
}
