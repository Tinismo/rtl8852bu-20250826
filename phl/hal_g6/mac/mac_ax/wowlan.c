		      ...
		      struct rtw_tcp_keepalive_parm *parm)
{
	...
	PLTFM_MSG_TRACE("[TCP_Keepalive] thresh(%d), timeout_full(%d sec), srv_cli_port(%d,%d)\n",
			parm->pld_size_thresh, parm->recv_timeout_full, parm->srv_port,
			parm->cli_port);
	PLTFM_MSG_TRACE("[TCP_Keepalive] ip0(%d), ip1(%d sec), ip2(%d), ip3(%d)\n",
			parm->srv_ip_0, parm->srv_ip_1, parm->srv_ip_2, parm->srv_ip_3);
	PLTFM_MSG_TRACE("[TCP_Keepalive] ip0(%d), ip1(%d sec), ip2(%d), ip3(%d)\n",
			parm->cli_ip_0, parm->cli_ip_1, parm->cli_ip_2, parm->cli_ip_3);
	...
	cpu_to_le32(SET_WORD(parm->ack_pktid,
		    ...
		    (parm->chk_srv_ip ?
			     FWCMD_H2C_TCP_KEEPALIVE_CHK_SRV_IP : 0) |
		    (parm->chk_srv_port ?
			     FWCMD_H2C_TCP_KEEPALIVE_CHK_SRV_PORT : 0) |
		    (parm->chk_cli_ip ?
			     FWCMD_H2C_TCP_KEEPALIVE_CHK_CLI_IP : 0) |
		    (parm->chk_cli_port ?
			     FWCMD_H2C_TCP_KEEPALIVE_CHK_CLI_PORT : 0) |
		    SET_WORD(parm->pld_size_thresh,
			     FWCMD_H2C_TCP_KEEPALIVE_PLD_SIZE_THRESH));

	fwcmd_tcp_keepalive->dword3 =
	cpu_to_le32(SET_WORD(parm->recv_timeout_full,
			     FWCMD_H2C_TCP_KEEPALIVE_RECV_TIMEOUT_FULL) |
		    SET_WORD(parm->srv_port,
			     FWCMD_H2C_TCP_KEEPALIVE_SRV_PORT));

	fwcmd_tcp_keepalive->dword4 =
	cpu_to_le32(SET_WORD(parm->srv_ip_0,
			     FWCMD_H2C_TCP_KEEPALIVE_SRV_IP_0) |
		    SET_WORD(parm->srv_ip_1,
			     FWCMD_H2C_TCP_KEEPALIVE_SRV_IP_1) |
		    SET_WORD(parm->srv_ip_2,
			     FWCMD_H2C_TCP_KEEPALIVE_SRV_IP_2) |
		    SET_WORD(parm->srv_ip_3,
			     FWCMD_H2C_TCP_KEEPALIVE_SRV_IP_3));

	fwcmd_tcp_keepalive->dword5 =
	cpu_to_le32(SET_WORD(parm->cli_ip_0,
			     FWCMD_H2C_TCP_KEEPALIVE_CLI_IP_0) |
		    SET_WORD(parm->cli_ip_1,
			     FWCMD_H2C_TCP_KEEPALIVE_CLI_IP_1) |
		    SET_WORD(parm->cli_ip_2,
			     FWCMD_H2C_TCP_KEEPALIVE_CLI_IP_2) |
		    SET_WORD(parm->cli_ip_3,
			     FWCMD_H2C_TCP_KEEPALIVE_CLI_IP_3));

	fwcmd_tcp_keepalive->dword6 =
	cpu_to_le32(SET_WORD(parm->cli_port,
			     FWCMD_H2C_TCP_KEEPALIVE_CLI_PORT));
