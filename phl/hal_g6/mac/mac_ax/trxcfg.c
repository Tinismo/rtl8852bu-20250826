...

{
	...
	reg = (info->band) ? R_AX_WMAC_NAV_CTL_C1 : R_AX_WMAC_NAV_CTL;
	val32 = MAC_REG_R32(reg);
	...
	reg = (info->band) ? R_AX_SPECIAL_TX_SETTING_C1 : R_AX_SPECIAL_TX_SETTING;
	val32 = MAC_REG_R32(reg);
	if (info->bmc_nav_prot_en)
		val32 |= B_AX_BMC_NAV_PROTECT;
	else
		val32 &= ~((u32)B_AX_BMC_NAV_PROTECT);
	MAC_REG_W32(reg, val32);
	return MACSUCCESS;
}
