			 ...
			 struct mac_ax_adapter_info *set)
{
	...
	PLTFM_MEMCPY(adapter->drv_info->adpt_info.cust_proc_id.base_board_id,
 ...
 */
	...
	if (adapter->env_info.intf == MAC_AX_INTF_USB &&
		...
		p_ops->usb_stuck_detect(adapter);
		mac_determ_usb_flush(adapter);
	}
#endif
	...
	ret = mac_pkt_ofld_watchdog(adapter);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("mac_pkt_ofld_watchdog fail %d\n", ret);
		final_ret = ret;
	}
