...
static const u32 fwdl_8852a_debug_step_num = 38;
...
static const u32 fwdl_8852b_debug_step_num = 38;
...
static const u32 fwdl_8852c_debug_step_num = 39;
...
static u32 fwhdr_hdr_parser(struct mac_ax_adapter *adapter, struct fwhdr_hdr_t *hdr,
					...
					u32 *msskp_key_num)
{
	...
	*msskp_key_num = mss_hdr.keypair_num;
	return MACSUCCESS;
#else
	PLTFM_MSG_ERR("[FWDL][ERR][%s] MAC_FEAT_SECURE_BOOT Not Enable\n", __func__);
	return MACNOTSUP;
#endif
...
static u32 update_fw_ver(struct mac_ax_adapter *adapter,
...
u32 mac_get_dynamic_hdr_ax(struct mac_ax_adapter *adapter, u8 *fw, u32 fw_len)
{
	...
	for (val32 = 0; val32 < dynamic_hdr_count; val32++) {
		...
		case FWDL_DYNAMIC_HDR_OUTSRC_GIT_INFO:
			...
			break;
		case FWDL_DYNAMIC_HDR_FW_COLLOCATION_VER:
			collocation_ver = le32_to_cpu(*(u32 *)content);
			adapter->fw_info.collocation_ver = collocation_ver;
			PLTFM_MSG_ALWAYS("[FWDL] FW Collocation Version = %08X\n", collocation_ver);
			break;
			    ...
			    u8 *fw, u32 hdr_len, u8 redl)
{
	...
	#endif
#if MAC_AX_PHL_H2C
	h2cb = h2cb_alloc(adapter, H2CB_TYPE_DATA);
#else
...
#if MAC_AX_PHL_H2C
		h2cb = h2cb_alloc(adapter, H2CB_TYPE_LONG_DATA);
#else
...
static u32 fwdl_patch_fw_delay(struct mac_ax_adapter *adapter)
{
	u32 fw_cpu_clk_addr = 0;
	u32 fw_fake_cpu_clk = 0;
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	/* Work Around
	 * During FWDL, Secure Checksum can not be bypass
	 * Since Secure Checksum envolved with OTP PWR Enable, which has a delay(1MS)
	 * Those delay is inaccureate, due to wrong CPU CLK setting in FW
	 * Assembly Code of FW will also affect the delay
	 *
	 * We shall not bypass Secure Checksum, so we will modify th CPU CLK setting
	 * by indirect access to IDMEM
	 */
	if (adapter->fw_info.is_sec_ic) {
		PLTFM_MSG_WARN("[WARN]%s: In security mode, bypass patch\n", __func__);
		return MACSUCCESS;
	}

	if (FWDL_IS_8852B) {
#if FWDL_SUPPORT_8852B
		fw_cpu_clk_addr = 0x18e0c3d8;
		fw_fake_cpu_clk = 0x0000000A;
#endif
	} else if (FWDL_IS_8851B) {
#if FWDL_SUPPORT_8851B
		fw_cpu_clk_addr = 0x18e0c374;
		fw_fake_cpu_clk = 0x0000000A;
#endif
	} else if (FWDL_IS_8852BT) {
#if FWDL_SUPPORT_8852BT
		fw_cpu_clk_addr = 0x18e0c49c;
		fw_fake_cpu_clk = 0x0000000A;
#endif
	} else if (FWDL_IS_8852C) {
#if FWDL_SUPPORT_8852C
		fw_cpu_clk_addr = 0x2020fd40;
		fw_fake_cpu_clk = 0x0000003C;
#endif
	} else if (FWDL_IS_8852D) {
#if FWDL_SUPPORT_8852D
		fw_cpu_clk_addr = 0x20210b80;
		fw_fake_cpu_clk = 0x0000003C;
#endif
	}
	...
	PLTFM_MUTEX_LOCK(&adapter->lock_info.ind_access_lock);
	adapter->dbg_info.ind_aces_cnt++;
	MAC_REG_W32(R_AX_FILTER_MODEL_ADDR, fw_cpu_clk_addr);
	MAC_REG_W32(R_AX_INDIR_ACCESS_ENTRY, fw_fake_cpu_clk);
	adapter->dbg_info.ind_aces_cnt--;
	PLTFM_MUTEX_UNLOCK(&adapter->lock_info.ind_access_lock);
...
static u32 fwdl_phase0(struct mac_ax_adapter *adapter)
{
	...
	FWDL_TIMESTAMP_BUFF.ts1 = PLTFM_GET_CURRENT_TIME_US();

	ret = fwdl_patch_fw_delay(adapter);
	if (ret)
		return ret;
		       ...
		       u8 *fw, u32 hdr_len, u8 redl)
{
	...
	FWDL_TIMESTAMP_BUFF.ts2 = PLTFM_GET_CURRENT_TIME_US();
...
static inline void fwdl_fw_debug_status_log(struct mac_ax_adapter *adapter, u16 debug_step)
{
	...
	case MAC_AX_CHIP_ID_8852BT:
		cur_fwdl_debug_step_num = fwdl_8852b_debug_step_num;
	...
	case MAC_AX_CHIP_ID_8192XB:
		cur_fwdl_debug_step_num = fwdl_8852c_debug_step_num;
	...
	if (debug_step >= cur_fwdl_debug_step_num) {
		PLTFM_MSG_ALWAYS("[FWDL] Unknown FW Debug Step 0x%X\n", debug_step);
		return;
	}
	...
	while (--cnt) {
		...
		if (cur_bootstep != pre_bootstep) {
			...
			if (!FWDL_TIMESTAMP_BUFF.ts3 &&
				...
				FWDL_TIMESTAMP_BUFF.ts3 = PLTFM_GET_CURRENT_TIME_US();
			if (!FWDL_TIMESTAMP_BUFF.ts4 &&
			    pre_bootstep >= FWDL_DBGSTEP_FW_BOOT_RAM_LOADED)
				FWDL_TIMESTAMP_BUFF.ts4 = PLTFM_GET_CURRENT_TIME_US();
		}
		...
		if (val8 == FWDL_WCPU_FW_INIT_RDY) {
			FWDL_TIMESTAMP_BUFF.ts5 = PLTFM_GET_CURRENT_TIME_US();
			break;
...
u32 mac_fwdl(struct mac_ax_adapter *adapter, u8 *fw, u32 len)
{
	...
	while (retry_cnt < (u8)FWDL_TRY_CNT) {
...
u32 mac_enable_cpu(struct mac_ax_adapter *adapter, u8 boot_reason, u8 dlfw)
{
	...
	FWDL_TIMESTAMP_BUFF.ts0 = PLTFM_GET_CURRENT_TIME_US();
...
u32 mac_query_fw_buff(struct mac_ax_adapter *adapter, enum rtw_fw_type cat, u8 **fw, u32 *fw_len)
{
	...
	chip_id = adapter->drv_info->sw_chip_id;
