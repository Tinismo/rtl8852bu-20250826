...
static u8 _diag_bb_type_mapping(u8 bb_type)
{
	u8 phl_diag_bb_type = PHL_DIAG_BB_EVT_UNDEF;

	switch (bb_type) {
	case F_FA_CNT:
		phl_diag_bb_type = PHL_DIAG_BB_EVT_FA_CNT;
		break;
	case F_DIG:
		phl_diag_bb_type = PHL_DIAG_BB_EVT_DIG;
		break;
	default:
		...
		u8 level, u8 version, u8 *buf, u32 len)
{
	...
	rtw_phl_send_diag_hub_msg(phl_com, PHL_DIAG_EVT_BB,
		_diag_bb_type_mapping(type), level, version, buf, len);
}
#endif
...
static enum bb_ch_mode_t _chinfo_ch_mode2bb(enum chinfo_ch_mode mode)
{
	switch(mode) {
	case CH_INFO_LEGACY_CH:
		return BB_CH_LEGACY_CH;
	case CH_INFO_MIMO_CH:
		return BB_CH_MIMO_CH;
	default:
...
_halchinfo2bbchinfo(struct chinfo_bbcr_cfg *cfg, struct bb_ch_info_cr_cfg_info *bb_cfg)
{
	...
	bb_cfg->ch_i_type = _chinfo_ch_mode2bb(cfg->ch_i_type);
...
u32 rtw_hal_bb_ch_info_max_ele_bmp(struct hal_info_t *hal_info, enum chinfo_ch_mode mode)
{
	return halbb_ch_info_ic_cfg(hal_info->bb, _chinfo_ch_mode2bb(mode));
}
...
rtw_hal_bb_cfg_rx_path(struct rtw_hal_com_t *hal_com, u8 rx_path, u8 phy_idx)
{
	...
	if(halbb_ctrl_rx_path(hal_info->bb, rx_path, phy_idx)) {
		...
		PHL_WARN("[HAL] status = RTW_HAL_STATUS_FAILURE !!!\n");
	}
...
rtw_hal_bb_cfg_tx_path(struct rtw_hal_com_t *hal_com, u8 tx_path, u8 phy_idx)
{
	...
	if(halbb_ctrl_tx_path_pmac(hal_info->bb, tx_path, phy_idx) != true)
		PHL_WARN("[HAL] status = RTW_HAL_STATUS_FAILURE !!!\n");
...
void rtw_hal_bb_auto_debug_en_phy_util(void *hal, bool en)
{
...
void rtw_hal_bb_auto_debug_en_phy_pmac(void *hal, bool en)
{
	struct hal_info_t *hal_info = NULL;
	...
	if (!hal)
		return;
	hal_info = (struct hal_info_t *)hal;
	halbb_auto_debug_en(hal_info->bb, AUTO_DBG_STORE_PMAC, en);
}
...
enum rtw_hal_status rtw_hal_bb_parse_diag_info(void *hal,
	struct rtw_phl_diag_bb_stat_v1 *stat, u8 *pmac_buf, u8 *util_buf)
{
	struct bb_bkp_phy_utility_info *bb_util = NULL;
	struct bb_bkp_pmac_info *bb_pmac = NULL;

	if (!hal || !stat || !pmac_buf || !util_buf)
		return RTW_HAL_STATUS_FAILURE;

	bb_util = (struct bb_bkp_phy_utility_info *)util_buf;
	bb_pmac = (struct bb_bkp_pmac_info *)pmac_buf;

	stat->rx_util = bb_util->rx_utility;
	stat->rx_avg_phy_rate = bb_util->avg_phy_rate;
	stat->rx_rate_plur = bb_util->rx_rate_plurality;
	stat->tx_rate = bb_util->tx_rate;
	stat->evm_1ss = bb_util->bb_physts_avg_i.evm_1ss;
	stat->evm_min = bb_util->bb_physts_avg_i.evm_min;
	stat->evm_max = bb_util->bb_physts_avg_i.evm_max;
	stat->snr = bb_util->bb_physts_avg_i.snr_avg;
	stat->cck_mac_txen = bb_pmac->bb_tx_cnt_i.cck_mac_txen;
	stat->cck_phy_txon = bb_pmac->bb_tx_cnt_i.cck_phy_txon;
	stat->ofdm_mac_txen = bb_pmac->bb_tx_cnt_i.ofdm_mac_txen;
	stat->ofdm_phy_txon = bb_pmac->bb_tx_cnt_i.ofdm_phy_txon;
	stat->cck_cca = bb_pmac->bb_cca_i.cnt_cck_cca;
	stat->ofdm_cca = bb_pmac->bb_cca_i.cnt_ofdm_cca;
	stat->cck_fa = bb_pmac->bb_fa_i.cnt_cck_fail;
	stat->ofdm_fa = bb_pmac->bb_fa_i.cnt_ofdm_fail;
	stat->valid = true;

	return RTW_HAL_STATUS_SUCCESS;
}

void rtw_hal_bb_diag_en(void *hal, bool en)
{
	if (!hal)
		return;

	rtw_hal_bb_auto_debug_en_phy_pmac(hal, en);
	rtw_hal_bb_auto_debug_en_phy_util(hal, en);
}
#endif
