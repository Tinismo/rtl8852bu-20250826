...
#ifdef CONFIG_QOS_MG
enum rtw_phl_qm_cap {
	RTW_QM_QOS_MAP_SUP = BIT0,
	RTW_QM_MSCS_SUP = BIT1,
	RTW_QM_SCS_SUP = BIT2,
	RTW_QM_DSCP_POL_SUP = BIT3,
	RTW_QM_SCS_QOS_CHARAC_SUP = BIT4 /* SCS with QoS Characteristics */
};
#endif /* CONFIG_QOS_MG */
...
struct phl_h2c_pkt_alloc_cnt {
	_os_atomic busy_h2c_pkt_cmd_cnt;
	_os_atomic busy_h2c_pkt_data_cnt;
	_os_atomic busy_h2c_pkt_ldata_cnt;
	_os_atomic h2c_query_cnt;
	_os_atomic h2c_mac_cnt;
	_os_atomic h2c_bb_cnt;
	_os_atomic h2c_rf_cnt;
	_os_atomic h2c_btc_cnt;
};

enum rtw_phl_comm_module {
	RTW_MODULE_MAC,	/* 0 */
	RTW_MODULE_BB,	/* 1 */
	RTW_MODULE_RF,	/* 2 */
	RTW_MODULE_BTC,	/* 3 */
...
#define PHL_MACID_MAX_ARRAY_NUM 8
/*#define PHL_MACID_MAX_NUM (PHL_MACID_MAX_ARRAY_NUM * 32)*//* 8x32=256 */
#define PHL_MACID_MAX_NUM PHL_MAX_STA_NUM
...
enum rtw_phl_send_msg_status {
	SEND_MSG_STS_UNSEND = 0,
	SEND_MSG_STS_SEND_OK,
	SEND_MSG_STS_SEND_FAIL
};
...
struct chg_opch_param {
	...
	enum rtw_phl_status cmd_start_sts; /* enum rtw_phl_status */
	bool start_msg_done;
	bool send_end_msg_fail;
	_os_lock lock;
...
#define regd_name_max_len 10
#define regd_name_max_size 32
#define regd_name_max_len_6g (regd_name_max_len + 4) /* + "_VLP" or "_LPI" or "_STD" */
#define regd_name_max_size_6g 64

struct ext_regd {
	char name[regd_name_max_len];
};

struct ext_regd_6g {
	char name[regd_name_max_len_6g];
};
...
#define MAX_NUM_PWRLMT_EXT_REGD_MAP 32

struct pwrlmt_ext_regd_map {
	u16 domain;
	char country[2];
	char reg_name[regd_name_max_len];
};
...
struct rtw_para_pwrlmt_info_t {
	...
	void *ext_regd_name; /* ext_regd or ext_regd_6g dynammic allocated array */
	u32 ext_regd_name_size;
#else
	char ext_regd_name[regd_name_max_size_6g][regd_name_max_len_6g];
#endif
	u16 ext_regd_arridx;
	u16 ext_reg_map_num;
	u8 *ext_reg_codemap;
...
struct rtw_phl_evt_ops {
	...
	/* core process ppdu sniff info */
	u32 (*os_process_snif_info)(void *priv, void *r, u16 *sz);
#endif

	void (*tx_power_tbl_loaded)(void *drv_priv, bool target_loaded, bool limit_loaded);

#ifdef CONFIG_PHL_CHANNEL_INFO_DIRECT_INDICATE
	void (*csi_indicate)(struct csi_ind_t *csi_ind);
#endif
};
...
enum link_state {
	...
	PHL_ClIENT_PS, /* For AP Role, SUT enter ps.*/
	PHL_ClIENT_ACTIVE, /* For AP Role, SUT leave from ps*/
	PHL_LINK_UP_NOA,
	PHL_LINK_CHG_CH,
	PHL_LINK_UNKNOWN
};
...
enum rlink_bcn_stop_rson {
	RLINK_BCN_STOP_RSON_DEFAULT,
	RLINK_BCN_STOP_RSON_CORE,
	RLINK_BCN_STOP_RSON_MAX
};

enum wr_status{
	WR_STATUS_SUSPEND = BIT0,
	WR_STATUS_HW_ALLOC_FAIL = BIT1,
	WR_STATUS_LSN_DISCOV = BIT2,
	WR_STATUS_LAT_SEN = BIT6, /* latency sensitive */
	WR_STATUS_MAX = BIT7
};
...
struct rtw_multi_mu_edca_param {
	u8 num;
...
struct rtw_bcn_offset {
	...
	u32 cr_tbtt_shift; /* us, CR current setting */
};
...
struct rtw_bcn_dist {
	...
	The baseline is not mod(TSF, B_Intvl) = 0 but is TBTT */
	u16 min;
	u16 max;
	u16 outlier_num;
	u16 outlier_l;
...
/*
 * @info: store array. info[0]: store Mod, TU, info[1]: store RSSI
 * @bcn_drift: The distance between TBTT and Bcn. Unit: TU
 */
struct rtw_bcn_short_i {
	...
	u16 bcn_drift[MAX_SHORT_INFO_BCN_NUM];
};

/*
 * @num: the store noumber of "info" array
 * @idx: store current index of "info" array
 * @info: store array. info[0]: store tsf, info[1]: store tbtt(us), info[2]: store hw rx time, info[3]: current sym time
 */
struct rtw_bcn_long_i {
	u8 num;
	u8 idx;
	u64 info[BCN_LONG_TYPE][MAX_LONG_INFO_BCN_NUM];
};

/*
 * Store rx bcn info
 * @pkt_len: pkt length of bcn
 * @app_tsf_t: the difference time betwwen TBTT and fill TSF field
 * @bcn_intvl: Bcn Intvl
 * @rate: data rate of bcn
 * @last_num_per_watchdog: rx bcn num in the last durtion of watchdog
 * @num_per_watchdog: counter the rx bcn each watchdog.
 * @bcn_s_i: short info of bcn, store simple information
 * @bcn_l_i: long bcn info, store full information
 ...
 * @bcn_dist_i: beacon distribution info
 * @cfg: Suggest setting of bcn tracking
 */
struct rtw_rx_bcn_info {
	...
	struct rtw_bcn_long_i bcn_l_i;
	struct rtw_bcn_offset offset_i;
	struct rtw_bcn_dist bcn_dist_i;
	struct rtw_bcn_tracking_cfg cfg;
};

#ifdef BCN_TRACKING_TEST
/* test case = 1, bcn intvl = 100,
    test case = 2, bcn intvl = 102,
*/
#define T_B_CASE 2
#define T_B_INTVL 102
...
enum phl_msg_evt_id {
	...
	MSG_EVT_WOW_WAKE_RSN = 15,
	MSG_EVT_BCN_RESEND = 16,
	MSG_EVT_DUMP_PLE_BUFFER = 17,
	MSG_EVT_MP_RX_PHYSTS = 18,
	MSG_EVT_ROLE_NTFY = 19,
	MSG_EVT_RX_PSTS = 20,
	MSG_EVT_SWCH_START = 21,
	MSG_EVT_SWCH_DONE = 22,
	MSG_EVT_DISCONNECT_PREPARE = 23,
	MSG_EVT_DISCONNECT = 24,
	MSG_EVT_DISCONNECT_END = 25,
	MSG_EVT_TSF_SYNC_DONE = 26,
	MSG_EVT_TX_RESUME = 27,
	MSG_EVT_AP_START_PREPARE = 28,
	MSG_EVT_AP_START = 29,
	MSG_EVT_AP_START_END = 30,
	MSG_EVT_AP_STOP_PREPARE = 31,
	MSG_EVT_AP_STOP = 32,
	MSG_EVT_AP_STOP_END = 33,
	...
	MSG_EVT_SET_MACID_PAUSE_AC = 154,
	MSG_EVT_CLIENT_PS_ANNC = 155,

	/* CONFIG_RTW_AP_BS_DECT */
	MSG_EVT_BCCHG_COUNTDOWN_ZERO = 156,

	/* Regulation*/
	MSG_EVT_REGU_SET_CNTRY_CODE = 157,
	MSG_EVT_SET_TAS_EN = 158,

	MSG_EVT_EDCCA_CFG = 160,

	/* TWT */
	MSG_EVT_TWT_STA_ACCEPT = 170,
	MSG_EVT_TWT_STA_TEARDOWN = 171,
	MSG_EVT_TWT_GET_TWT = 172,
	MSG_EVT_TWT_WAIT_ANNOUNCE = 173,
	MSG_EVT_TWT_INFO_F_HDR = 185,
	/* TWT end */

	MSG_EVT_GET_USB_MODE_STATUS = 190,
	MSG_EVT_GET_U3_PERF_MODE = 191,
#ifdef CONFIG_PHL_CUSTOM_FEATURE_USB
	MSG_EVT_SET_USB_SW_ABILITY = 192,
#endif

#ifdef CONFIG_PHL_WKARD_REDUCE_SER
	/* usb toggle for ser*/
	MSG_EVT_USB_TOGGLE = 170,
#endif
	/* dbg */
	...
	/* For Multi Role Connection optimization */
	...
	/* NAN */
	MSG_EVT_NAN_START = 250,
	MSG_EVT_NAN_START_END = 251,
	MSG_EVT_NAN_STOP = 252,
	MSG_EVT_NAN_STOP_END = 253,
	MSG_EVT_NAN_ENTRY = 254,
	/* NAN end */
	...
	MSG_EVT_USR_FRAME_ACT_RPT = 411,

	/* MP FT2 Test */
	MSG_EVT_FT2_L12_TEST = 420,

	#ifdef CONFIG_DBCC_P2P_BG_LISTEN
	MSG_EVT_CONNECT_END_DBCC_EN = 500,
	MSG_EVT_DISCONNECT_END_DBCC_EN = 501,
	MSG_EVT_CONNECT_CMD_DBCC_DIS = 502,
	MSG_EVT_DISCONNECT_CMD_DBCC_EN = 503,
	#endif

	/* tx pwr limit config*/
	MSG_EVT_SET_PWR_LIMIT_STD = 600,
	MSG_EVT_SET_PWR_LIMIT_LOW = 601,
	MSG_EVT_SET_PWR_LIMIT_VLOW = 602,

	/* PS start */
	MSG_EVT_PS_CAP_CHG = 701,
	MSG_EVT_PS_PERIOD_CHK = 702,
	MSG_EVT_PS_DBG_CMD = 703,
	MSG_EVT_PS_LPS_ENTER = 704,
	MSG_EVT_PS_LPS_LEAVE = 705,
	MSG_EVT_PS_HAL_PWR_REQ = 706,
	MSG_EVT_PS_PWR_CFG_ENTER = 749,
	MSG_EVT_PS_PWR_CFG_LEAVE = 750,
	/* PS end */
...
/* length of supported_rates */
#define RATE_SET_LEN(_rate_set) \
		(u8)MIN(_os_strlen(_rate_set), sizeof(_rate_set))
...
struct protocol_cap_t {
	...
	u8 max_amsdu_num:2; /*  0: no lim, 1: 32, 2: 16, 3: 8 */
...
#ifdef CONFIG_QOS_MG
	u16 qm_cap; /* enum rtw_phl_qm_cap */
#endif /* CONFIG_QOS_MG */
...
#ifdef CONFIG_PCI_HCI
enum RTW_PHL_PCIE_L1DLY{
	RTW_PHL_PCIE_L1DLY_16US = 0,
	RTW_PHL_PCIE_L1DLY_32US = 1,
	RTW_PHL_PCIE_L1DLY_64US = 2,
	RTW_PHL_PCIE_L1DLY_INFI = 3,
	RTW_PHL_PCIE_L1DLY_R_ERR = 0xFD,
	RTW_PHL_PCIE_L1DLY_DEF = 0xFE,
	RTW_PHL_PCIE_L1DLY_IGNORE = 0xFF
};

enum RTW_PHL_PCIE_L0SDLY{
	RTW_PHL_PCIE_L0SDLY_1US = 0,
	RTW_PHL_PCIE_L0SDLY_2US = 1,
	RTW_PHL_PCIE_L0SDLY_3US = 2,
	RTW_PHL_PCIE_L0SDLY_4US = 3,
	RTW_PHL_PCIE_L0SDLY_5US = 4,
	RTW_PHL_PCIE_L0SDLY_6US = 5,
	RTW_PHL_PCIE_L0SDLY_7US = 6,
	RTW_PHL_PCIE_L0SDLY_R_ERR = 0xFD,
	RTW_PHL_PCIE_L0SDLY_DEF = 0xFE,
	RTW_PHL_PCIE_L0SDLY_IGNORE = 0xFF
};

enum RTW_PHL_PCIE_CLKDLY{
	RTW_PHL_PCIE_CLKDLY_0 = 0,
	RTW_PHL_PCIE_CLKDLY_5US = 1,
	RTW_PHL_PCIE_CLKDLY_6US = 2,
	RTW_PHL_PCIE_CLKDLY_11US = 3,
	RTW_PHL_PCIE_CLKDLY_15US = 4,
	RTW_PHL_PCIE_CLKDLY_19US = 5,
	RTW_PHL_PCIE_CLKDLY_25US = 6,
	RTW_PHL_PCIE_CLKDLY_30US = 7,
	RTW_PHL_PCIE_CLKDLY_38US = 8,
	RTW_PHL_PCIE_CLKDLY_50US = 9,
	RTW_PHL_PCIE_CLKDLY_64US = 10,
	RTW_PHL_PCIE_CLKDLY_100US = 11,
	RTW_PHL_PCIE_CLKDLY_128US = 12,
	RTW_PHL_PCIE_CLKDLY_150US = 13,
	RTW_PHL_PCIE_CLKDLY_192US = 14,
	RTW_PHL_PCIE_CLKDLY_200US = 15,
	RTW_PHL_PCIE_CLKDLY_300US = 16,
	RTW_PHL_PCIE_CLKDLY_400US = 17,
	RTW_PHL_PCIE_CLKDLY_500US = 18,
	RTW_PHL_PCIE_CLKDLY_1MS = 19,
	RTW_PHL_PCIE_CLKDLY_3MS = 20,
	RTW_PHL_PCIE_CLKDLY_5MS = 21,
	RTW_PHL_PCIE_CLKDLY_10MS = 22,
	RTW_PHL_PCIE_CLKDLY_V1_0 = 23,
	RTW_PHL_PCIE_CLKDLY_V1_16US = 24,
	RTW_PHL_PCIE_CLKDLY_V1_32US = 25,
	RTW_PHL_PCIE_CLKDLY_V1_64US = 26,
	RTW_PHL_PCIE_CLKDLY_V1_80US = 27,
	RTW_PHL_PCIE_CLKDLY_V1_96US = 28,
	RTW_PHL_PCIE_CLKDLY_R_ERR = 0xFD,
	RTW_PHL_PCIE_CLKDLY_DEF = 0xFE,
	RTW_PHL_PCIE_CLKDLY_IGNORE = 0xFF
};


struct rtw_pcie_cfgspc_param {
...
struct rtw_phl_btwt_i {
	u8 rtwt_tfc_prsnt : 1; /* Restricted TWT Traffic Info Present */
	u8 rsvd : 2;
	u8 btwt_id : 5;
	u8 btwt_prstnc; /* Broadcast TWT Persistence */
};

struct rtw_phl_btwt_req_type {
	u8 twt_request;
	enum rtw_phl_setup_cmd twt_setup_cmd; /*twt setup command*/
	u8 trigger;
	u8 lst_bc_para_set; /* Last Broadcast Parameter Set */
	enum rtw_phl_flow_type flow_type;
	u8 btwt_rcmd; /* Broadcast TWT Recommendation */
	u8 twt_wake_int_exp; /*twt wake interval exponent*/
	u8 rsvd;
};

struct rtw_phl_bcast_twt_para_set {
	struct rtw_phl_btwt_req_type req_type;
	u32 target_wake_t_h;
	u32 target_wake_t_l;
	u8 nom_min_twt_wake_dur; /*nominal minimum twt wake duration*/
	u16 twt_wake_int_mantissa; /*twt wake interval mantissa*/
	struct rtw_phl_btwt_i btwt_i;
};
...
/* element info*/
	...
	union {
		...
		struct rtw_phl_bcast_twt_para_set b_twt_para_set[MAX_BTWT_PARA_SET];
	} info;
};
...
/* TWT Information start */
struct rtw_phl_twt_info_f {
	u8 twt_flow_id;
	u8 rsp_req;
	u8 next_twt_req;
	u8 next_twt_size;
...
struct rtw_phl_twt_info_f_hdr_i{
	struct rtw_phl_stainfo_t *sta;
	struct rtw_phl_twt_info_f info_f;
};
...
struct rtw_ps_cap_t {
	...
	u8 lps_force_tx;
	u8 lps_intensive_trx_lvl;
	bool lps_intensive_chk;

	bool ps_pause_tx; /* cannot tx under low power state */
	/* wow lps */
	u8 lps_wow_en;
	u8 lps_wow_cap;
...
enum dump_cfg_option {
	DUMP_CFG_IO_ALLOW = BIT0,
	DUMP_CFG_FW_LOG_ALLOW = BIT1,
};

#define IS_IO_DUMP_ALLOWED(_phl_com) (_phl_com->dbg_cfg.dump_cfg & DUMP_CFG_IO_ALLOW)
#define SET_IO_DUMP_ALLOWED(_phl_com) (_phl_com->dbg_cfg.dump_cfg |= DUMP_CFG_IO_ALLOW)
#define CLR_IO_DUMP_ALLOWED(_phl_com) (_phl_com->dbg_cfg.dump_cfg &= ~DUMP_CFG_IO_ALLOW)
#define IS_FW_LOG_DUMP_ALLOWED(_phl_com) (_phl_com->dbg_cfg.dump_cfg & DUMP_CFG_FW_LOG_ALLOW)
...
enum rtw_btc_esoc_type {
	BTC_EXT_SOC_NONE,
	BTC_EXT_SOC_8761,
	BTC_EXT_SOC_8771,
	BTC_EXT_SOC_SILAB_MG21,
	BTC_EXT_SOC_NORDI_NRF52840,
	BTC_EXT_SOC_MAX
};
...
struct rtw_phl_dbg_mac {
	u8 mac_wow_fw_dbg;
};

struct rtw_phl_dbg_cfg_t {
	/*PHL or MISC section*/
	...
	/*HALBB section*/
	struct rtw_phl_dbg_bb bb_cfg;
	/* HALMAC section */
	struct mac_ax_fw_log fw_log_info;
	struct rtw_phl_dbg_mac mac_cfg;

	/* refs. enum dump_cfg_option */
	u8 dump_cfg;
...
struct dev_cap_t {
	...
	u8 domain_6g;
	u8 btc_mode;
	enum rtw_btc_esoc_type btc_esoc_type;
	u8 btc_ant_iso_db;
#ifdef CONFIG_BTCOEX
	...
	u8 rpq_tmr; /* unit: 32us, 0: no adjust, use mac default size: 0xFF */
	bool quota_turbo;
	u8 band_sup;
	u8 bw_sup;
	...
	bool efuse_dump_ofld;
	bool adie_efuse_dump_ofld;
#endif

	u8 disable_dyn_txpwr; /* Whether to disable Dynamic Transmit Power Control. default 0 */
...
#endif
	RX_FLTR_TYPE_MODE_MONITOR,
#ifdef CONFIG_PHL_TEST_MP
	RX_FLTR_TYPE_MODE_MP,
#endif
	RX_FLTR_TYPE_MODE_MAX
};

enum rtw_lat_sen_chk {
	LAT_SEN_CHK_NONE = 0,
	LAT_SEN_CHK_LPS_TX,
	LAT_SEN_CHK_BCN_TRACKING
};
...
struct rtw_wifi_role_link_t {
	...
	u8 ext_hw_port;                /*Another MAC HW Port */
	enum rlink_status status;
	u8 bcn_stop;			/* bitmap for rlink_bcn_stop_rson */
	...
	struct phl_queue ext_sta_queue;
};
...
struct rtw_wifi_role_t {
	...
	struct phl_queue ext_mld_queue;
	u8 rlink_num;
...
struct rtw_stats {
	...
	u32 rx_err_cnt; /* icv or crc32 err */
	u32 rx_ok_cnt;
	/* ltr */
...
#define MAX_SCS_DESCRIPTOR 5
#define MAX_TCLAS_ELE 3
#define MAX_TCLAS_CLASSIFIER_FILTER 20
#define MAX_DOM_NAME_LEN 50
#define MAX_QOS_MG_ELE_NUM 4
#define MAX_TEMP_SCS_STATUS 10
#define MAX_SCS_INFO_BYTES 354
...
enum scs_req_status {
	...
	TCLAS_PROCESSING_TERMINATED_INSUFFICIENT_QOS = 128,
	TCLAS_PROCESSING_TERMINATED_POLICY_CONFLICT = 129,	/*Requested TCLAS processing has been terminated by the AP due to conflict with higher layer QoS policies*/
	SCS_REQ_TCLAS_LENGHT_FAULT = 254,
	SCS_REQ_TCLAS_PROCESSING_FAULT = 255
};
...
enum rtw_phl_scs_req_tp {
	SCS_REQ_ADD = 0,
	SCS_REQ_REMOVE = 1,
	SCS_REQ_CHG = 2
};

struct rtw_phl_tclas_ipv4 {
	u8 src_ip[4];
	u8 dst_ip[4];
};

struct rtw_phl_tclas_ipv6 {
	u8 src_ip[16];
	u8 dst_ip[16];
};
struct rtw_phl_tclas_type0 {
	u8 mask;
	u8 source_addr[6];
	u8 destination_addr[6];
	u16 type;
};

struct rtw_phl_tclas_type1 {
	u8 mask;
	u8 version;
	u8 src_ip[4];
	u8 dst_ip[4];
...
struct rtw_phl_tclas_type2 {
	u8 mask;
	u8 tci;
};
struct rtw_phl_tclas_type3 {
	u8 mask;
	u16 offset;
	u8 filter_value[MAX_TCLAS_CLASSIFIER_FILTER];
	u8 filter_mask[MAX_TCLAS_CLASSIFIER_FILTER];
};
struct rtw_phl_tclas_type4 {
	u8 mask;
	u8 version;
	union {
		struct rtw_phl_tclas_ipv4 v4;
		struct rtw_phl_tclas_ipv6 v6;
	}ip;
	u16 src_port;
	u16 dst_port;
	u8 dscp;
...
struct rtw_phl_tclas_type5 {
	u8 mask;
	u8 up_or_pcp;
	u8 dei;
	u8 vid;
};
struct rtw_phl_tclas_type6 {
	u32 mask;
	u32 frame_control;
	u32 duration;
	u8 addr1[12];
	u8 addr2[12];
	u8 addr3[12];
	u32 sequence_control;
	u8 addr4[12];
	u32 qos_control;
	u64 ht_control;
};
struct rtw_phl_tclas_type7 {
	u32 mask;
	u32 frame_control;
	u8 addr1[12];
	u8 addr2[12];
...
struct rtw_phl_tclas_type10 {
	u8 protocol_instance;
	u8 protocol_num_nexthdr;
	u8 filter_byte;
	u8 filter_value[MAX_TCLAS_CLASSIFIER_FILTER];
	u8 filter_mask[MAX_TCLAS_CLASSIFIER_FILTER];
};

struct rtw_phl_tclas_clf {
	u8 tp;
	union {
...
struct rtw_phl_intra_ac_priority {
	u8 ac_up : 3;
	u8 alternate_queue : 1;
	u8 drop_eligibility : 1;
	u8 reserved : 3;
};

struct rtw_phl_tclas_element {
	u8 valid; /* sta set valid bit */
	u8 up;
	u8 type;
	union {
	struct rtw_phl_tclas_type0 t0;
...
#define QOS_CHAR_MAX_MSDU_SZ_PRESN BIT0 /* Maximum MSDU Size */
#define QOS_CHAR_SER_ST_PRESN BIT1 /* Service Start Time */
#define QOS_CHAR_SER_ST_LNKID_PRESN BIT2 /* Service Start Time LinkID */
#define QOS_CHAR_MEAN_DATA_RATE_PRESN BIT3 /* Mean Data Rate */
#define QOS_CHAR_BURST_SZ_PRESN BIT4 /* Burst Size */
#define QOS_CHAR_MSDU_LIFE_T_PRESN BIT5 /* MSDU Lifetime */
#define QOS_CHAR_DELIVERY_RATIO_PRESN BIT6 /* MSDU Delivery Ratio */
#define QOS_CHAR_CNT_EXP_PRESN BIT7 /* MSDU Count Exponent */
#define QOS_CHAR_MED_T_PRESN BIT8 /* Medium Time */

struct rtw_phl_qos_ele {
	u8 valid;
	/* +++Control Info+++ */
	u32 direction : 2;
	u32 tid : 4;
	u32 up : 3;
	u32 presence_bitmap : 16;
	u32 link_id : 4;
	u32 reserved : 3;
	/* ---Control Info--- */
	...
	u8 serv_st_lnkid; /* service_start_time_link_id */
	u32 mean_datarate : 24;
	u32 burst_size;
	u16 msdu_lifetime;
	u8 msdu_delivery_ratio;
	u8 msdu_count_exponet;
	u16 medium_time;
};


struct rtw_phl_scs_info {
	u8 scsid;
	u8 req_type; /* enum rtw_phl_scs_req_tp */
	/* Intra-Access Category Priority Element */
	u8 iac_valid;
	struct rtw_phl_intra_ac_priority iacp;
	/* TCLAS Elements */
	struct rtw_phl_tclas_element tclas[MAX_TCLAS_ELE];
	/* TCLAS Processing Element */
	u8 processing;
	/* QoS Characteristics Element*/
	struct rtw_phl_qos_ele qos_ele;
};

struct rtw_phl_temp_scs_status {
	u8 scsid;
	u8 status;
	u8 req_type;
};

enum rtw_phl_wfa_cap {
	QOSMG_DSCP_POL = BIT0, /* QoS Management DSCP Policy */
	QOSMG_USOL_DSCP_POL_ASSOC = BIT1, /* QoS Management Unsolicited DSCP Policy At Association */
	QOSMG_NONEHT_SCS_TFC_DESC = BIT2 /* Non-EHT SCS Traffic Description Support  */
};

#define MAX_SCS_LIST_NUM MAX_SCS_DESCRIPTOR
struct rtw_phl_scs_sts_duple {
	u8 scsid;
	u16 sts;
};

struct rtw_phl_scs_rsp_f {
	u8 dt;
	u8 cnt;
	struct rtw_phl_scs_sts_duple sts_list[MAX_SCS_LIST_NUM];
};

/* Wi-Fi Alliance Capabilities element */
#define MAX_WFA_CAP 1
struct rtw_phl_wfa_cap_ele {
	u8 cap[MAX_WFA_CAP]; /* enum rtw_phl_wfa_cap */
};

/* QosMAP */
#define MAX_DSCP_EXC_NUM 21
#define DSCP_RNG_NUM 8

struct rtw_phl_dscp_exc {
	u8 dscp;
	u8 up; /* user priority */
};
struct rtw_phl_dscp_rng {
	u8 dscp_l; /* DSCP Low Value */
	u8 dscp_h; /* DSCP High Value */
};

struct rtw_phl_qos_map_ele {
	u8 exc_num;
	struct rtw_phl_dscp_exc dscp_exc[MAX_DSCP_EXC_NUM];
	struct rtw_phl_dscp_rng dscp_rng[DSCP_RNG_NUM];
};

/* MSCS */
#define MAX_TCLAS_MASK_NUM 4
#define MIN_MSCS_DESC_LEN 10
struct rtw_phl_mscs_up_ctrl {
	u8 up_bitmap; /* User Priority Bitmap */
	u8 up_lim:3; /* User Priority Limit */
	u8 rsvd:5;
};

struct rtw_phl_tclas_mask {
	struct rtw_phl_tclas_clf clf;
};

struct rtw_phl_mscs_desc {
	enum rtw_phl_scs_req_tp req_tp; /**/
	struct rtw_phl_mscs_up_ctrl up_ctrl;
	u32 stm_timeout; /* stream timeout */
	u8 tclas_mask_num;
	struct rtw_phl_tclas_mask tclas_mask[MAX_TCLAS_MASK_NUM];
	/* optional subelements */
	u8 mscs_sts_valide;
	u16 mscs_sts; /* Status codes */
};

struct rtw_phl_mscs_rsp {
	u8 dt; /* Dialog Token */
	u16 sts; /* Status codes */
	u8 desc_valide;
	struct rtw_phl_mscs_desc desc;
};

/* DSCP Policy Start */
enum rtw_phl_dscp_pol_attr_req_tp {
	DSCP_POL_REQ_ADD = 0,
	DSCP_POL_REQ_UPDATE = 0,
	DSCP_POL_REQ_REMOVE = 1
};

struct rtw_phl_qos_mg_ele {
	...
	u8 dscp_attr_valid;
	u8 dscp_pol_id; /* DSCP Policy ID */
	enum rtw_phl_dscp_pol_attr_req_tp req_tp; /* Request Type */
	u8 dscp;
	/* TCLAS attribute */
	u8 tclas_attr_valid;
	struct rtw_phl_tclas_clf clf;
	...
	u8 dom_attr_valid;
	u8 dom_len;
	u8 dom_name[MAX_DOM_NAME_LEN]; /*domain name*/
};

struct rtw_phl_dscp_req_ctrl {
	u8 more:1;
	u8 reset:1;
	u8 rsvd:6;
};

struct rtw_phl_dscp_rsp_ctrl {
	u8 more:1;
	u8 reset:1;
	u8 rsvd:6;
};

/* DSCP Policy Query frame */
struct rtw_phl_dscp_pol_qur {
	u8 dt; /* Dialog Token */
	u8 qm_ele_num;
	struct rtw_phl_qos_mg_ele qm_ele[MAX_QOS_MG_ELE_NUM];
};

/* DSCP Policy Req frame */
struct rtw_phl_dscp_pol_req {
	u8 dt; /* Dialog Token */
	struct rtw_phl_dscp_req_ctrl ctrl; /* request control field */
	u8 qm_ele_num;
	struct rtw_phl_qos_mg_ele qm_ele[MAX_QOS_MG_ELE_NUM];
};
...
enum dscp_pol_sts {
	DSCP_POL_SUCCESS = 0,
	DSCP_POL_REQUEST_DECLINED = 1,
	DSCP_POL_REQUESTED_CLASSIFIER_NOT_SUPPORTED = 2,
	DSCP_POL_INSUFFICIENT_PROCESSING_RESOURCES = 3,
	DSCP_POL_STS_RSVD
};

struct rtw_phl_dcsp_sts {
	u8 id; /* dscp policy id */
	enum dscp_pol_sts sts;
};

/* DSCP Policy Rsp frame */
struct rtw_phl_dscp_pol_rsp {
	u8 dt; /* Dialog Token */
	struct rtw_phl_dscp_rsp_ctrl ctrl; /* response control field */
	u8 cnt;
	struct rtw_phl_dcsp_sts sts[MAX_QOS_MG_ELE_NUM];
};
...
#define MAX_QM_RULE_ROLE 2
#define MAX_QM_RULE 5

struct qm_ruleset {
	struct rtw_wifi_role_t *wr;
	struct qm_rule_i rule[MAX_QM_RULE];
};
...
#ifdef CONFIG_DRV_HDL_QM_POL
	_os_lock lock;
	struct qm_ruleset ruleset[MAX_QM_RULE_ROLE];
#endif /* CONFIG_DRV_HDL_QM_POL */
};

#endif /* CONFIG_QOS_MG */

#define HT_RX_REORDER_BUF_TIMEOUT_SHORT_MS 50
...
struct rtw_phl_stainfo_t {
	...
	_os_timer reorder_timer; /* reorder timer for all @tid_rx of the
	                          * stainfo */
	u16 reorder_timeout; /* reorder timeout value of all @tid_rx */
...
#ifdef CONFIG_QOS_MG
	struct rtw_phl_scs_info scs_info[MAX_SCS_DESCRIPTOR];
	struct rtw_phl_temp_scs_status scs_temp[MAX_TEMP_SCS_STATUS];
#endif /* CONFIG_QOS_MG */

#ifdef CONFIG_DIG_TDMA
	enum rtw_p2p_app_type p2p_session;
#endif
	bool flag_pwr_diff_large;
	u8 ref_mac[MAC_ALEN];
	bool is_nontx;
};

#define PHL_USE_RA_BW_MODE
#define PHL_STA_TX_BW_MODE(sta) (((sta)->tx_bw_mode == CHANNEL_WIDTH_MAX) ? (sta)->chandef.bw : (sta)->tx_bw_mode)
...
#ifdef CONFIG_PHL_CHSWOFLD
struct chsw_ofld_info_t {
	u8 chsw_ofld_en;
	u8 rf_reload;
	u8 skip_normal_watchdog;
};
#endif
...
enum phl_physts_bitmap_t {
	PHL_PSTS_MAP_TD_SEARCH_FAIL = 0,
	PHL_PSTS_MAP_BRK_BY_TX_PKT = 1,
	PHL_PSTS_MAP_CCA_SPOOF = 2,
	PHL_PSTS_MAP_OFDM_BRK = 3,
	PHL_PSTS_MAP_CCK_BRK = 4,
	PHL_PSTS_MAP_DL_MU_SPOOFING = 5,
	PHL_PSTS_MAP_HE_MU = 6,
	PHL_PSTS_MAP_VHT_MU = 7,
	PHL_PSTS_MAP_UL_TB_SPOOFING = 8,
	PHL_PSTS_MAP_RSVD_9 = 9,
	PHL_PSTS_MAP_TRIG_BASE_PPDU = 10,
	PHL_PSTS_MAP_CCK_PKT = 11,
	PHL_PSTS_MAP_LEGACY_OFDM_PKT = 12,
	PHL_PSTS_MAP_HT_PKT = 13,
	PHL_PSTS_MAP_VHT_PKT = 14,
	PHL_PSTS_MAP_HE_PKT = 15,
	PHL_PSTS_MAP_PHYSTS_BITMAP_NUM
};


struct phl_plcp_hdr_l_sig_info {
	/* B0 */
	u8 rate:4;
	u8 rsvd_bit:1;
	u8 len0_l:3;
...
struct phl_plcp_hdr_ht_sig_1_info {
	/* B0 */
	u8 mcs:7;
	u8 cbw:1;
	/* B1 */
...
struct phl_plcp_hdr_ht_sig_2_info {
	/* B0 */
	u8 smoothing:1;
	u8 not_sounding:1;
	u8 rsvd_bit:1;
...
struct phl_plcp_hdr_vht_sig_a1_info {
	/* B0 */
	u8 bw:2;
	u8 rsvd_bit:1;
	u8 stbc:1;
...
struct phl_plcp_hdr_vht_sig_a1_ext_su_info {
	/* B0 */
	u8 su_sts:3;
	u8 paid_l:5;
	/* B1 */
...
struct phl_plcp_hdr_vht_sig_a1_ext_mu_info {
	/*B0*/
	u8 mu_sts_0:3;
	u8 mu_sts_1:3;
	u8 mu_sts_2_l:2;
...
struct phl_plcp_hdr_vht_sig_a2_info {
	/* B0 */
	u8 sgi:1;
	u8 sgi_disamb:1;
	u8 su_mu_0_coding:1;
...
struct phl_plcp_hdr_vht_sig_a2_ext_su_info {
	/* B0 */
	u8 su_vht_mcs:4;
	u8 bf:1;
	u8 rsvd_0:3;
...
struct phl_plcp_hdr_vht_sig_a2_ext_mu_info {
	/* B0 */
	u8 mu_coding_1:1;
	u8 mu_coding_2:1;
	u8 mu_coding_3:1;
...
struct phl_plcp_hdr_he_sig_a1_su_info {
	/*B0*/
	u8 fmt:1;
	u8 beam_chg:1;
	u8 ul_dl:1;
...
struct phl_plcp_hdr_he_sig_a2_su_info {
	/*B0*/
	u8 txop:7;
	u8 coding:1;
	/*B1*/
...
struct phl_plcp_hdr_he_sig_a1_mu_info {
	/*B0*/
	u8 ul_dl:1;
	u8 he_sig_b_mcs:3;
	u8 he_sig_b_dcm:1;
...
struct phl_plcp_hdr_he_sig_a2_mu_info {
	/*B0*/
	u8 txop:7;
	u8 rsvd_1:1;
	/*B1*/
...
struct phl_snif_info {
	bool snif_mode_en;
#define SNIF_I_RATE_MODE_LEGACY 0
#define SNIF_I_RATE_MODE_HT 1
#define SNIF_I_RATE_MODE_VHT 2
#define SNIF_I_RATE_MODE_HE 3
	u8 snif_rate_mode; /*0~3: Legacy/HT/VHT/HE*/
	u16 snif_rate;
	enum phl_physts_bitmap_t snif_ie_bitmap_type;
	/* LGCY/HT/VHT/HE */
	u32 *snif_l_sig;
...
#ifdef CONFIG_PHL_SNIFFER_SUPPORT
#define SNIFFER_INFO_MODE_NORMAL 0
#define SNIFFER_INFO_MODE_HIGH_PERFORMANCE 1
	u8 sniffer_info_mode;
#endif
};
...
enum rtw_phl_thermal_status{
	RTW_PHL_THERMAL_STATUS_BELOW_THRESHOLD,
	RTW_PHL_THERMAL_STATUS_STAY_THRESHOLD,
	RTW_PHL_THERMAL_STATUS_ABOVE_THRESHOLD,
	RTW_PHL_THERMAL_STATUS_UNKNOWN
};
...
struct rtw_phl_smart_ant_info_t{
	u16 tx_rate;
	u16 rx_rate;
	u16 pkt_cnt_cck;
	u16 pkt_cnt_ofdm;
	...
	s8 edcca;
	u8 nhm_pwr;
	struct acs_mntr_rpt acs_rpt[MAX_ACS_INFO];
};
#endif

#ifdef DBG_MONITOR_TIME
enum phl_time_flags {
	TIME_PHL_INIT, /*rtw_phl_init,*/
	TIME_PHL_PRELOAD, /*rtw_phl_preload,*/
	TIME_HAL_FAST_START,  /*hal_fast_start*/
	TIME_HAL_GET_EFUSE, /*hal_get_efuse*/
	TIME_HAL_FAST_STOP, /*hal_fast_stop*/

	TIME_PHL_START, /*rtw_phl_start*/
	TIME_HAL_START, /*rtw_hal_start*/

	TIME_HAL_MAC_HAL_INIT,   /*rtw_hal_mac_hal_init*/
	TIME_HAL_EFUSE_PROC,   /*rtw_hal_efuse_process*/
	TIME_HAL_INIT_BB_REG1,   /*rtw_hal_init_bb_early_init*/
	TIME_HAL_INIT_BB_REG2,   /*rtw_hal_init_bb_reg*/
	TIME_HAL_INIT_RF_REG,   /*rtw_hal_init_rf_reg*/

	TIME_HAL_INIT_BTC,   /*rtw_hal_btc_init_coex_cfg_ntfy*/

	TIME_HAL_BB_DM_INIT, /*rtw_hal_bb_dm_init*/
	TIME_HAL_RF_DM_INIT, /*rtw_hal_rf_dm_init*/

	/*TIME_PHL_SET_CHAN,*/ /*phl_set_ch_bw_ex*/
	TIME_HAL_SET_CHAN, /*rtw_hal_set_ch_bw*/
	TIME_HAL_RFK, /*rtw_hal_rf_chl_rfk_trigger*/

	TIME_PHL_SUSPEND, /*rtw_phl_suspend*/
	TIME_PHL_RESUME,  /*rtw_phl_resume*/
	TIME_PHL_WOW_START,   /*phl_wow_start*/
	TIME_PHL_WOW_STOP, /*phl_wow_stop*/

	TIME_PHL_ROLE_SUSPEND, /*phl_cmd_role_suspend*/
	TIME_PHL_ROLE_RECOVER, /*phl_cmd_role_recover*/

	TIME_PHL_ROLE_ALLOC, /*rtw_phl_wifi_role_alloc*/
	TIME_PHL_ROLE_FREE, /*rtw_phl_wifi_role_free*/

	TIME_PHL_STOP, /*rtw_phl_stop,*/

	TIME_PHL_DEINIT, /*rtw_phl_deinit,*/

	/*keep last*/
	TIME_PHL_MAX
};
#define PHL_TIME_FLAGS_SZ (TIME_PHL_MAX + 1)
#endif
...
struct rtw_phl_com_t {
	...
	struct rtw_phl_dbg_cfg_t dbg_cfg; /* SW control debug flow */
...
#ifdef CONFIG_PHL_CHSWOFLD
	struct chsw_ofld_info_t chsw_ofld_info;
#endif

	/* Flags to control/check DecryptCustomFile */
	bool decrypt_cf;
...
#ifdef CONFIG_SMART_ANTENNA
	struct rtw_phl_smart_ant_info_t ant_info;
#endif
...
#ifdef CONFIG_USB_HCI
#ifdef CONFIG_PHL_WKARD_REDUCE_SER
	ATOMIC_T usb_tx_flush_mode;
	ATOMIC_T usb_write_port_complete;
	ATOMIC_T usb_write_port_cnt;
#endif
#endif
#endif
};
...
struct rtw_keep_alive_param {
	u8 sta_chk_rx_bmp;
	u8 sta_chk_tx_bmp;
	u32 ap_chk_sta_bmp[MAX_WIFI_ROLE_NUMBER];
};
#endif
...
struct rtw_bcn_early_rpt {
	u8 band; /* band idx 0~1 */
	u8 port; /* port idx 0~4 */
};
#endif
