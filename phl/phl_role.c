...
enum rtw_phl_status
phl_release_ext_hw_port(struct phl_info_t *phl,
			struct rtw_wifi_role_link_t *rlink)
{
	struct rtw_phl_com_t *phl_com = phl->phl_com;
	struct mr_ctl_t *mr_ctl = phlcom_to_mr_ctrl(phl_com);
	...
	if (HW_PORT_MAX != rlink->ext_hw_port) {
		...
		_os_spinlock(phl_to_drvpriv(phl), &band_ctrl->lock, _bh, NULL);
		band_ctrl->port_map &= ~BIT(rlink->ext_hw_port);
		_os_spinunlock(phl_to_drvpriv(phl), &band_ctrl->lock, _bh, NULL);
		rlink->ext_hw_port = HW_PORT_MAX;
	}
...
enum rtw_phl_status
phl_get_available_hw_port(struct phl_info_t *phl, u8 hw_band, u8 *port_num)
{
	u8 psts = RTW_PHL_STATUS_FAILURE;
	struct rtw_phl_com_t *phl_com = phl->phl_com;
	struct mr_ctl_t *mr_ctl = phlcom_to_mr_ctrl(phl_com);
	struct hal_spec_t *hal_spec = phl_get_ic_spec(phl_com);
	struct hw_band_ctl_t *band_ctrl;
	...
	for ((*port_num) = 0; (*port_num) < hal_spec->port_num; (*port_num)++) {
				...
				u8 hw_band)
{
	...
	{
		psts = phl_get_available_hw_port(phl, hw_band, &port_num);
                           ...
                           bool ignore_hw_fail)
{
	...
	/* alloc mld for self */
	mld = phl_alloc_mld(phl_info, role, mac_addr, dtype, false);
	if (mld == NULL) {
...
static enum rtw_phl_status _phl_rlink_set_bcn_early_rpt(struct phl_info_t *phl,
							struct rtw_wifi_role_link_t *rlink,
							u8 en)
{
...
)
{
		...
		for (idx = 0; idx < wrole->rlink_num; idx++) {
			...
			if (sta) {
				if (type == PHL_RTYPE_TDLS || wrole->type == PHL_RTYPE_TDLS) {
					wrole->type = type;
					pstate = RTW_PHL_STATUS_SUCCESS;
				} else if (type == PHL_RTYPE_MONITOR) {
					wrole->type = type;
					pstate = rtw_phl_enter_mon_mode(phl_info, wrole);
					if (pstate != RTW_PHL_STATUS_SUCCESS)
						PHL_ERR("%s change role to monitor fail\n", __func__);
				} else {
					if (wrole->type == PHL_RTYPE_MONITOR) {
						pstate = rtw_phl_leave_mon_mode(phl_info, wrole);
						if (pstate != RTW_PHL_STATUS_SUCCESS)
							PHL_ERR("%s leave monitor fail\n", __func__);
					}
					wrole->type = type;
					wrole->mstate = MLME_NO_LINK;
	...
	case WR_CHG_LSN_DISCOV:
		if (wrole->type == PHL_RTYPE_P2P_DEVICE) {
			...
			pstate = phl_mr_info_upt(phl_info, rlink);
			if (RTW_PHL_STATUS_SUCCESS == pstate)
				pstate = rtw_phl_mr_rx_filter_opt(phl_info, rlink);
		}
		break;
	...
	if (RTW_PHL_STATUS_SUCCESS != pstate)
		PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "%s: Failed, chg_id(%d)\n",
			  __func__, chg_id);
	return pstate;
}
...
_phl_wifi_role_stop(struct phl_info_t *phl_i, struct rtw_wifi_role_t *wrole)
{
	...
	for (lidx = 0; lidx < wrole->rlink_num; lidx++) {
		rlink = get_rlink(wrole, lidx);
		if (RTW_HAL_STATUS_SUCCESS != rtw_hal_port_deinit(phl_i->hal, rlink, false)) {
			PHL_ERR("%s wr deinit failed for link idx(%d), port idx(%d), it may influence power consumption\n",
				__func__, lidx, rlink->hw_port);
			psts = RTW_PHL_STATUS_FAILURE;
		}
		if (rlink->ext_hw_port == HW_PORT_MAX)
			continue;
		if (RTW_HAL_STATUS_SUCCESS != rtw_hal_port_deinit(phl_i->hal, rlink, true)) {
			PHL_ERR("%s wr deinit failed for link idx(%d), port idx(%d), it may influence power consumption\n",
				__func__, lidx, rlink->ext_hw_port);
			psts = RTW_PHL_STATUS_FAILURE;
		}
	}
_exit:
