...
void phl_cmd_scanofld_end(void *drv, void *_param, u8 band_idx)
{
	...
	if(TEST_SCAN_FLAG(param->state, band_idx, CMD_SCAN_STARTED) &&
	   !TEST_SCAN_FLAG(param->state, band_idx, CMD_SCAN_DF_IO) ) {
		...
		rtw_hal_set_rxfltr_opt_by_mode(phl_info->hal, band_idx,
		                               sctrl->fltr_mode);
...
error:
	if (indicate) {
		...
		param->sctrl_num = 0;
		param->rxfltr_mode = RX_FLTR_OPT_MODE_NONE;
	}
...
_cmd_scanofld_start(struct phl_info_t *phl_info,
		    ...
		    u8 band_idx)
{
	rtw_hal_set_rxfltr_by_scan(phl_info->hal, band_idx, &sctrl->fltr_mode);
	...
	case MSG_EVT_SCANOFLD_SWITCH:
		...
		if (sctrl->scan_ch != NULL) {
			...
			if (param->ops->scan_ch_ready) {
				cur_chdef.chan = (u8)scan_ch->channel;
				cur_chdef.bw = scan_ch->bw;
				cur_chdef.offset = scan_ch->offset;
				cur_chdef.band = scan_ch->band;
				cur_chdef.center_ch = rtw_phl_get_center_ch(&cur_chdef);
				rtw_hal_sync_cur_ch(phl_info->hal, band_idx, cur_chdef);
...
u8 rtw_phl_scanofld_support(void *phl)
{
	...
	return phl_info->phl_com->dev_cap.scan_ofld;
}
#endif
