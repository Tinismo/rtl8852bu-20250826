...
static bool phl_set_pw_lmt_regu(struct phl_info_t *phl_info, struct txpwr_regu_info_t *conf, const char *caller)
{
	...
	if (conf->force) {
		if (conf->regu_2g_len && (conf->regu_2g_len != pr_info->regu_2g_len || !pr_info->regu_2g)) {
		...
		if (conf->regu_5g_len && (conf->regu_5g_len != pr_info->regu_5g_len || !pr_info->regu_5g)) {
		...
		if (conf->regu_6g_len && (conf->regu_6g_len != pr_info->regu_6g_len || !pr_info->regu_6g)) {
		...
		if (conf->regu_2g_len != pr_info->regu_2g_len
			|| (pr_info->regu_2g_len && !pr_info->regu_2g)
		...
		if (conf->regu_5g_len != pr_info->regu_5g_len
			|| (pr_info->regu_5g_len && !pr_info->regu_5g)
		...
		if (conf->regu_6g_len != pr_info->regu_6g_len
			|| (pr_info->regu_6g_len && !pr_info->regu_6g)
...
bool rtw_phl_check_tpe_allow(void *phl, struct rtw_tpe_info_t *tpe_info)
{
	...
	status = rtw_hal_check_tpe_allow(phl_info->hal, tpe_info);
	if (!status)
		PHL_ERR("%s rtw_hal_check_tpe_allow: status = %u\n", __func__, status);
	...
	const struct phl_pwr_byrate_stbl_sel **stbls, u8 *stbl_num)
{
	...
	rtw_hal_get_pwr_byrate_stbls(phl_info->hal, stbls, stbl_num);
}
	...
	struct phl_pwr_byrate_val_sel sel)
{
	...
	return rtw_hal_get_pwr_byrate_val_raw_tbl(phl_info->hal, sel);
}
	...
	struct phl_pwr_byrate_offset_sel sel)
{
	...
	return rtw_hal_get_pwr_byrate_offset_raw_tbl(phl_info->hal, sel);
}
