...
#endif
		/* notify "before" power cfg when entering low power */
			...
			u8 cur_pwr_lvl, u8 req_pwr_lvl, u8 cfg_ok)
{
	...
	if (cur_pwr_lvl > req_pwr_lvl) { /* enter ps */
		if (!cfg_ok) { /* fail */
			/* notify "after" power cfg when entering low power fail */
			if (req_pwr_lvl == PS_PWR_LVL_PWROFF) { /* driver ips */
				...
				#endif
				rtw_hal_notification(phl_info->hal,
				                     MSG_EVT_PS_PWR_CFG_LEAVE,
				                     HW_BAND_0);
			}
		}
	} else { /* leave ps */
		if (cfg_ok) { /* ok */
			/* notify "after" power cfg when leaving low power */
...
phl_ps_cfg_pwr_lvl(struct phl_info_t *phl_info, u8 ps_mode, u8 cur_pwr_lvl, u8 req_pwr_lvl)
{
	...
	if (cur_pwr_lvl == PS_PWR_LVL_PWR_GATED && hstatus == RTW_HAL_STATUS_SUCCESS)
