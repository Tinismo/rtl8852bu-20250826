...
rtw_phl_dump_h2c_pool_alloc_stats(void *phl)
{
...
#ifdef USB_XMIT_THREAD_MODE
		if(rtw_hal_is_inited(phl_com, phl_info->hal)) {
			if (_os_atomic_read(d, &phl_com->usb_write_port_complete)) {
				if (_os_atomic_read(d, &phl_com->usb_write_port_cnt) == 0) {
                    rtw_phl_cmd_usb_toggle_flush_for_ser(phl_info, 1, &flush_mode, HW_BAND_0, PHL_CMD_DIRECTLY, 0);
		    /*
                    if (_os_atomic_read(d, &phl_com->usb_tx_flush_mode) != flush_mode) {
                        _os_atomic_set(d, &phl_com->usb_tx_flush_mode, flush_mode);
                        if (flush_mode)
                            RTW_INFO("%s enable usb tx flush mode\n", __func__);
                        else
                            RTW_INFO("%s disable usb tx flush mode\n", __func__);
                    }
		    */
                }
                _os_atomic_set(d, &phl_com->usb_write_port_complete, 0);
            }
        }
...
static void _phl_free_h2c_pool(struct phl_info_t *phl_info)
{
	...
	if (NULL != h2c_pkt_pool) {
		...
		pq_deinit(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_cmd_list);
		pq_deinit(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_data_list);
		pq_deinit(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_ldata_list);
		pq_deinit(drv_priv, &h2c_pkt_pool->busy_h2c_pkt_list);
...
_phl_alloc_h2c_pool(struct phl_info_t *phl_info)
{
	...
	if (NULL != h2c_pkt_pool) {
		...
		pq_init(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_cmd_list);
		pq_init(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_data_list);
		pq_init(drv_priv, &h2c_pkt_pool->idle_h2c_pkt_ldata_list);
		pq_init(drv_priv, &h2c_pkt_pool->busy_h2c_pkt_list);
